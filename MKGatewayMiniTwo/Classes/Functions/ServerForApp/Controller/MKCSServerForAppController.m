...
@interface MKCSServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCSServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKCSServerForAppModel *dataModel;

@property (nonatomic, strong)MKCSServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKCSServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKCSServerForAppController

- (void)dealloc {
    NSLog(@"MKCSServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKCSMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKCSMQTTDataManager shared] connect];
}
...
#pragma mark - MKCSServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)cs_selectedServerParams:(NSString *)fileName {
    [MKCSExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCSServerForAppModel *model = [MKCSServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKCSMQTTDataManager shared].state == MKCSMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKCSMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_cs_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKCSExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKCSMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKCSMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSServerForAppController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCSServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCSServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKCSServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCSServerConfigAppFooterViewModel alloc] init];
    }
