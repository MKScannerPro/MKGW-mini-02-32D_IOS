...
@implementation MKCSSyncDeviceCellModel
@end

@interface MKCSSyncDeviceCell ()
...
@implementation MKCSSyncDeviceCell

+ (MKCSSyncDeviceCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSSyncDeviceCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSSyncDeviceCellIdenty"];
    if (!cell) {
        cell = [[MKCSSyncDeviceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSSyncDeviceCellIdenty"];
    }
    return cell;
}
...
- (void)backBtnPressed {
    ...
    [self updateIcon];
    if ([self.delegate respondsToSelector:@selector(cs_syncDeviceCell_selected:index:)]) {
        [self.delegate cs_syncDeviceCell_selected:self.backBtn.selected index:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCSSyncDeviceCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSSyncDeviceCellModel.class]) {
        return;
    }
...
- (void)updateIcon {
    UIImage *btnIcon = (self.backBtn.selected ? LOADICON(@"MKGatewayMiniTwo", @"MKCSSyncDeviceCell", @"cs_listButtonSelectedIcon.png") : LOADICON(@"MKGatewayMiniTwo", @"MKCSSyncDeviceCell", @"cs_listButtonUnselectedIcon.png"));
