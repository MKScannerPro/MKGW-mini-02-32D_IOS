...
@interface MKCSSyncDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKCSSyncDeviceCellDelegate>
...
@implementation MKCSSyncDeviceController

- (void)dealloc {
    NSLog(@"MKCSSyncDeviceController销毁");
}
...
- (void)rightButtonMethod {
    ...
    [alert showWithAccount:[MKCSUserLoginManager shared].username completeBlock:^{
        [[MKCSUserLoginManager shared] syncLoginDataWithHome:[MKCSUserLoginManager shared].isHome username:[MKCSUserLoginManager shared].username password:@""];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCSSyncDeviceCell *cell = [MKCSSyncDeviceCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKCSSyncDeviceCellDelegate
- (void)cs_syncDeviceCell_selected:(BOOL)selected index:(NSInteger)index {
    MKCSSyncDeviceCellModel *cellModel = self.dataList[index];
...
- (void)syncButtonPressed {
    ...
    for (MKCSSyncDeviceCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            MKCSCreatScannerProDeviceModel *uploadModel = [[MKCSCreatScannerProDeviceModel alloc] init];
    ...
    [[MKCSNetworkService share] addScannerProDevicesToCloud:uploadList isHome:[MKCSUserLoginManager shared].isHome token:self.token sucBlock:^(id returnData) {
...
- (void)loadDataSections {
    for (NSInteger i = 0; i < self.deviceList.count; i ++) {
        MKCSDeviceModel *device = self.deviceList[i];
        MKCSSyncDeviceCellModel *cellModel = [[MKCSSyncDeviceCellModel alloc] init];
        cellModel.index = i;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSSyncDeviceController", @"cs_authorIcon.png") forState:UIControlStateNormal];
