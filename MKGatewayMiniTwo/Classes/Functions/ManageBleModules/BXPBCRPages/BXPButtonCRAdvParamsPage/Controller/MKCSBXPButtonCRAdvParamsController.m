//
//  MKCSBXPButtonCRAdvParamsController.m
...
@interface MKCSBXPButtonCRAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBXPButtonCRAdvParamsModel *dataModel;

@end

@implementation MKCSBXPButtonCRAdvParamsController

- (void)dealloc {
    NSLog(@"MKCSBXPButtonCRAdvParamsController销毁");
}
...
- (void)cs_advNormalCell_setPressed:(NSInteger)index
    ...
    [MKCSMQTTInterface cs_bxpBtnCRConfigAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if (!ValidDict(advParams)) {
        return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSBXPButtonCRAdvParamsControllerCell"];
    }
...
- (MKCSBXPButtonCRAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBXPButtonCRAdvParamsModel alloc] init];
    }
    return _dataModel;
}
