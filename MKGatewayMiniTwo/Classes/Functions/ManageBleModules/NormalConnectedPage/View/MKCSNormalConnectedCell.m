...
@implementation MKCSNormalConnectedCellModel
...
@interface MKCSNormalConnectedCell ()
...
@implementation MKCSNormalConnectedCell

+ (MKCSNormalConnectedCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSNormalConnectedCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSNormalConnectedCellIdenty"];
    if (!cell) {
        cell = [[MKCSNormalConnectedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSNormalConnectedCellIdenty"];
    }
    return cell;
}
...
- (void)writeButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_normalConnectedCell_writeButtonPressed:row:serverUUID:characteristic:)]) {
        [self.delegate cs_normalConnectedCell_writeButtonPressed:self.dataModel.section
...
- (void)readButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_normalConnectedCell_readButtonPressed:row:serverUUID:characteristic:)]) {
        [self.delegate cs_normalConnectedCell_readButtonPressed:self.dataModel.section
                                                            row:self.dataModel.row
                                                     serverUUID:self.dataModel.server
                                                 characteristic:self.dataModel.characteristic];
    }
...
- (void)notifyButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_normalConnectedCell_notifyButtonPressed:section:row:serverUUID:characteristic:)]) {
        [self.delegate cs_normalConnectedCell_notifyButtonPressed:!self.notifyButton.selected
...
#pragma mark - setter
- (void)setDataModel:(MKCSNormalConnectedCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSNormalConnectedCellModel.class]) {
        return;
    }
    ...
    [self.notifyButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSNormalConnectedCell", @"cs_characteristic_notifyIcon.png") forState:UIControlStateNormal];
    self.notifyButton.selected = NO;
    if (_dataModel.notifyStatus == 1) {
        //打开了监听
        [self.notifyButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSNormalConnectedCell", @"cs_characteristic_receiveIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)writeButton {
    if (!_writeButton) {
        ...
        [_writeButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSNormalConnectedCell", @"cs_characteristic_writeIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)readButton {
    if (!_readButton) {
        ...
        [_readButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSNormalConnectedCell", @"cs_characteristic_readIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)notifyButton {
    if (!_notifyButton) {
        ...
        [_notifyButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSNormalConnectedCell", @"cs_characteristic_notifyIcon.png") forState:UIControlStateNormal];
