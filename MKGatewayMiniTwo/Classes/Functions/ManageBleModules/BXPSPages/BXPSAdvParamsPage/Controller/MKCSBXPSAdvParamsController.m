...
@interface MKCSBXPSAdvParamsController ()<UITableViewDelegate,
UITableViewDataSource,
MKCSBXPSAdvNormalCellDelegate,
MKCSBXPSAdvTriggerCellDelegate,
MKCSBXPSAdvTriggerTwoStateCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKCSBXPSAdvParamsModel *dataModel;

@end

@implementation MKCSBXPSAdvParamsController

- (void)dealloc {
    NSLog(@"MKCSBXPSAdvParamsController销毁");
}
...
#pragma mark - MKCSBXPSAdvNormalCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
...
- (void)cs_BXPSAdvNormalCell_setPressed:(NSInteger)index
    ...
    [MKCSMQTTInterface cs_configBXPSAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
#pragma mark - MKCSBXPSAdvTriggerCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
///   - interval: 当前ADV interval
...
/*
 ...
 6
 */
- (void)cs_BXPSAdvTriggerCell_setPressed:(NSInteger)index
                                interval:(NSString *)interval
                                 txPower:(NSInteger)txPower {
    if (!ValidStr(interval) || [interval integerValue] < 1 || [interval integerValue] > 100) {
        [self.view showCentralToast:@"ADV interval Error"];
...
#pragma mark - MKCSBXPSAdvTriggerTwoStateCellDelegate
/// set按钮点击事件
/// - Parameters:
///   - index: index
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:MKCSBXPSAdvNormalCellModel.class]) {
        //正常广播
        MKCSBXPSAdvNormalCell *cell = [MKCSBXPSAdvNormalCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    if ([obj isKindOfClass:MKCSBXPSAdvTriggerCellModel.class]) {
        //触发广播
        MKCSBXPSAdvTriggerCell *cell = [MKCSBXPSAdvTriggerCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    if ([obj isKindOfClass:MKCSBXPSAdvTriggerTwoStateCellModel.class]) {
        //触发前+触发后广播
        MKCSBXPSAdvTriggerTwoStateCell *cell = [MKCSBXPSAdvTriggerTwoStateCell initCellWithTableView:self.tableView];
        cell.dataModel = obj;
        cell.delegate = self;
        return cell;
    }
    return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSBXPSAdvParamsControllerIdenty"];
}
...
- (CGFloat)loadTableCellHeight:(NSIndexPath *)indexPath {
    ...
    if ([obj isKindOfClass:MKCSBXPSAdvNormalCellModel.class]) {
        //正常广播
        MKCSBXPSAdvNormalCellModel *tempModel = (MKCSBXPSAdvNormalCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    if ([obj isKindOfClass:MKCSBXPSAdvTriggerCellModel.class]) {
        //触发广播
        MKCSBXPSAdvTriggerCellModel *tempModel = (MKCSBXPSAdvTriggerCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    if ([obj isKindOfClass:MKCSBXPSAdvTriggerTwoStateCellModel.class]) {
        MKCSBXPSAdvTriggerTwoStateCellModel *tempModel = (MKCSBXPSAdvTriggerTwoStateCellModel *)obj;
        return [tempModel fetchCellHeight];
    }
    return 0.0f;
}
...
- (void)loadSectionDatas {
    for (NSInteger i = 0; i < self.dataModel.dataList.count; i ++) {
        ...
        if (!enable) {
            //通道关闭 No Data
            MKCSBXPSAdvNormalCellModel *cellModel = [[MKCSBXPSAdvNormalCellModel alloc] init];
            cellModel.slotIndex = channel;
            cellModel.slotType = MKCSBXPSAdvNormalCellSlotTypeNoData;
            ...
            if (channelType == 0) {
                ...
                MKCSBXPSAdvTriggerCellModel *cellModel = [[MKCSBXPSAdvTriggerCellModel alloc] init];
                cellModel.slotIndex = channel;
                ...
                NSDictionary *afterAdv = dic[@"trigger_after_adv"];
                MKCSBXPSAdvTriggerTwoStateCellModel *cellModel = [[MKCSBXPSAdvTriggerTwoStateCellModel alloc] init];
                cellModel.slotIndex = channel;
...
- (MKCSBXPSAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBXPSAdvParamsModel alloc] init];
    }
    return _dataModel;
}
