...
@implementation MKCSBXPSAdvTriggerCellModel

- (CGFloat)fetchCellHeight {
    if (self.slotType == MKCSBXPSAdvTriggerCellSlotTypeNoData) {
        return 44.f;
    }
    return 150.f;
}
...
@interface MKCSBXPSAdvTriggerCell ()
...
@implementation MKCSBXPSAdvTriggerCell

+ (MKCSBXPSAdvTriggerCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSBXPSAdvTriggerCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSBXPSAdvTriggerCellIdenty"];
    if (!cell) {
        cell = [[MKCSBXPSAdvTriggerCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSBXPSAdvTriggerCellIdenty"];
    }
    return cell;
}
...
- (void)layoutSubviews {
    ...
    if (self.dataModel.slotType == MKCSBXPSAdvTriggerCellSlotTypeNoData) {
...
- (void)setButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cs_BXPSAdvTriggerCell_setPressed:interval:txPower:)]) {
        [self.delegate cs_BXPSAdvTriggerCell_setPressed:self.dataModel.slotIndex
...
#pragma mark - setter
- (void)setDataModel:(MKCSBXPSAdvTriggerCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSBXPSAdvTriggerCellModel.class]) {
        return;
    }
    ...
    if (_dataModel.slotType == MKCSBXPSAdvTriggerCellSlotTypeNoData) {
...
- (NSString *)getSlotTypeString:(MKCSBXPSAdvTriggerCellSlotType)slotType {
    if (slotType == MKCSBXPSAdvTriggerCellSlotTypeUID) {
        return @"UID";
    }
    if (slotType == MKCSBXPSAdvTriggerCellSlotTypeURL) {
        return @"URL";
    }
    if (slotType == MKCSBXPSAdvTriggerCellSlotTypeTLM) {
        return @"TLM";
    }
    if (slotType == MKCSBXPSAdvTriggerCellSlotTypeBeacon) {
        return @"iBeacon";
    }
    if (slotType == MKCSBXPSAdvTriggerCellSlotTypeSensorInfo) {
        return @"Sensor info";
    }
    if (slotType == MKCSBXPSAdvTriggerCellSlotTypeTHInfo) {
        return @"T&H info";
    }
