//
//  MKCSBXPSTHDataSampleRateModel.m
...
@interface MKCSBXPSTHDataSampleRateModel ()
...
@implementation MKCSBXPSTHDataSampleRateModel
...
- (BOOL)readTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKCSMQTTInterface cs_readBXPSTHDataSampleRateWithBleMacAddress:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKCSMQTTInterface cs_configBXPSSampleRate:[self.self.sampleRate integerValue] bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
