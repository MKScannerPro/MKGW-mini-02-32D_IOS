//
//  MKCSBXPSRealTimeTHDataController.m
...
@interface MKCSBXPSRealTimeTHDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCSBXPSRealTimeTHDataController

- (void)dealloc {
    NSLog(@"MKCSBXPSRealTimeTHDataController销毁");
...
- (void)cs_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCSMQTTInterface cs_bxpBXPSNotifyRealTimeHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCSReceiveBXPSRealTimeHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
