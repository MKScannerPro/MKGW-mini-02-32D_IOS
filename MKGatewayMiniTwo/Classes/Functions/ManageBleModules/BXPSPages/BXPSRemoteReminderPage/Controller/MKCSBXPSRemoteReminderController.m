//
//  MKCSBXPSRemoteReminderController.m
...
@interface MKCSBXPSRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBXPSRemoteReminderModel *dataModel;

@end

@implementation MKCSBXPSRemoteReminderController

- (void)dealloc {
    NSLog(@"MKCSBXPSRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKCSMQTTInterface cs_bxpBXPSLedRemoteReminderWithBleMac:self.bleMac color:self.dataModel.color blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCSBXPSRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBXPSRemoteReminderModel alloc] init];
    }
    return _dataModel;
}
