//
//  MKCSBXPSAccelerometerController.m
...
@interface MKCSBXPSAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCSBXPSAccelerometerController

- (void)dealloc {
    NSLog(@"MKCSBXPSAccelerometerController销毁");
...
- (void)cs_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCSMQTTInterface cs_bxpBXPSNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCSReceiveBXPSAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
