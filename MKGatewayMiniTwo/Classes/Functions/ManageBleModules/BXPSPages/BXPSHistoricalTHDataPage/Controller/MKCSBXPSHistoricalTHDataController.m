//
//  MKCSBXPSHistoricalTHDataController.m
...
@interface MKCSBXPSHistoricalTHDataController ()<MFMailComposeViewControllerDelegate,
MKCSBXPSHistoricalTHDataHeaderViewDelegate>

@property (nonatomic, strong)MKCSBXPSHistoricalTHDataHeaderView *headerView;
...
@implementation MKCSBXPSHistoricalTHDataController

- (void)dealloc {
    NSLog(@"MKCSBXPSHistoricalTHDataController销毁");
...
#pragma mark - MKCSBXPSHistoricalTHDataHeaderViewDelegate
- (void)cs_BXPSHistoricalHTDataHeaderView_syncButtonPressed:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_bxpBXPSNotifyHistoricalHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCSReceiveBXPSHistoricalHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
...
- (void)cs_BXPSHistoricalHTDataHeaderView_deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_bxpBXPSDeleteHistoricalHTDataWithBleMac:self.bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
        self.textView.text = @"";
    } failedBlock:^(NSError * _Nonnull error) {
...
#pragma mark - getter
- (MKCSBXPSHistoricalTHDataHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKCSBXPSHistoricalTHDataHeaderView alloc] init];
