//
//  MKCSBXPSHistoricalTHDataHeaderView.m
...
@interface MKCSBXPSHistoricalTHDataHeaderView ()
...
@implementation MKCSBXPSHistoricalTHDataHeaderView
...
- (void)syncButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_BXPSHistoricalHTDataHeaderView_syncButtonPressed:)]) {
        [self.delegate cs_BXPSHistoricalHTDataHeaderView_syncButtonPressed:!self.syncButton.selected];
    }
}

- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_BXPSHistoricalHTDataHeaderView_deleteButtonPressed)]) {
        [self.delegate cs_BXPSHistoricalHTDataHeaderView_deleteButtonPressed];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_BXPSHistoricalHTDataHeaderView_exportButtonPressed)]) {
        [self.delegate cs_BXPSHistoricalHTDataHeaderView_exportButtonPressed];
    }
}
...
- (UIImageView *)synIcon {
    if (!_synIcon) {
        ...
        _synIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPSHistoricalTHDataHeaderView", @"cs_threeAxisAcceLoadingIcon.png");
    }
    return _synIcon;
}
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPSHistoricalTHDataHeaderView", @"cs_slotExportEnableIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)deleteButton {
    if (!_deleteButton) {
        ...
        [_deleteButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPSHistoricalTHDataHeaderView", @"cs_slotExportDeleteIcon.png") forState:UIControlStateNormal];
