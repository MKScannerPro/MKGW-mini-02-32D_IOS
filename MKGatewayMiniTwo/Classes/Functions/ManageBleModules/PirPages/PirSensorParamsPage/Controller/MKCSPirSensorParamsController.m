//
//  MKCSPirSensorParamsController.m
...
@interface MKCSPirSensorParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSPirSensorParamsModel *dataModel;

@end

@implementation MKCSPirSensorParamsController

- (void)dealloc {
    NSLog(@"MKCSPirSensorParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSPirSensorParamsController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSPirSensorParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSPirSensorParamsModel alloc] init];
    }
    return _dataModel;
}
