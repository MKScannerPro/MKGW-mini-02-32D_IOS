...
@interface MKCSPirAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSPirAdvParamsModel *dataModel;

@end

@implementation MKCSPirAdvParamsController

- (void)dealloc {
    NSLog(@"MKCSPirAdvParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSPirAdvParamsController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSPirAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSPirAdvParamsModel alloc] init];
    }
    return _dataModel;
}
