...
@interface MKCSPirAdvParamsModel ()
...
@implementation MKCSPirAdvParamsModel
...
- (BOOL)readAdvParams:(NSString *)bleMac {
    ...
    [MKCSMQTTInterface cs_readMKPirAdvParamsWithBleMacAddress:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvParams:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configMKPirAdvParamsWithInterval:[self.interval integerValue] txPower:self.txPower bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
