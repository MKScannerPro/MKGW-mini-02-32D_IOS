...
@interface MKCSTofSensorDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCSTofSensorDataController

- (void)dealloc {
    NSLog(@"MKCSTofSensorDataController销毁");
...
- (void)cs_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCSMQTTInterface cs_bxpMKTofNotifySensorDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveSensorDatas:)
                                                         name:MKCSReceiveMKTofDistanceDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
