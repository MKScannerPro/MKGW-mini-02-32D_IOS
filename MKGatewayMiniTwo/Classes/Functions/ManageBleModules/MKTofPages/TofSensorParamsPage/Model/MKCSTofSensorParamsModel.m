...
@interface MKCSTofSensorParamsModel ()
...
@implementation MKCSTofSensorParamsModel
...
- (BOOL)readSensorParams:(NSString *)bleMac {
    ...
    [MKCSMQTTInterface cs_readMKTofSensorParamsWithBleMacAddress:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configSensorParams:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configMKTofSensorParamsWithInterval:[self.interval integerValue] sampleCount:[self.count integerValue] sampleTime:[self.time integerValue] bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readDistanceMode:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_readMKTofRangingModeWithBleMacAddress:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDistanceMode:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configMKTofRangingMode:self.distanceMode bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
