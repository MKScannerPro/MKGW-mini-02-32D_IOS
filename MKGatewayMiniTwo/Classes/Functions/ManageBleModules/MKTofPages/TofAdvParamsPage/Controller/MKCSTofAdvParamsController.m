...
@interface MKCSTofAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSTofAdvParamsModel *dataModel;

@end

@implementation MKCSTofAdvParamsController

- (void)dealloc {
    NSLog(@"MKCSTofAdvParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSTofAdvParamsController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSTofAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSTofAdvParamsModel alloc] init];
    }
    return _dataModel;
}
