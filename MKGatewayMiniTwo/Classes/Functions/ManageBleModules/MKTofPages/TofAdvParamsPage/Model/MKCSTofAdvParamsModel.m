...
@interface MKCSTofAdvParamsModel ()
...
@implementation MKCSTofAdvParamsModel
...
- (BOOL)readAdvParams:(NSString *)bleMac {
    ...
    [MKCSMQTTInterface cs_readMKTofAdvParamsWithBleMacAddress:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvParams:(NSString *)bleMac {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configMKTofAdvParamsWithInterval:[self.interval integerValue] txPower:self.txPower bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
