//
//  MKCSTofAccelerometerController.m
...
@interface MKCSTofAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCSTofAccelerometerController

- (void)dealloc {
    NSLog(@"MKCSTofAccelerometerController销毁");
...
- (void)cs_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCSMQTTInterface cs_bxpMKTofNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCSReceiveMKTofAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
