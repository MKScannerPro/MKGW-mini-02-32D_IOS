...
@interface MKCSBXPDAccParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBXPDAccParamsModel *dataModel;

@end

@implementation MKCSBXPDAccParamsController

- (void)dealloc {
    NSLog(@"MKCSBXPDAccParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPDAccParamsController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSBXPDAccParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBXPDAccParamsModel alloc] init];
    }
    return _dataModel;
}
