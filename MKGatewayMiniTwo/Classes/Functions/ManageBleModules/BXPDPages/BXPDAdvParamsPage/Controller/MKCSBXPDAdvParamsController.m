...
@interface MKCSBXPDAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSDAdvParamsModel *dataModel;

@end

@implementation MKCSBXPDAdvParamsController

- (void)dealloc {
    NSLog(@"MKCSBXPDAdvParamsController销毁");
}
...
- (void)cs_BXPAdvParamsCell_setPressedWithSlotIndex:(NSInteger)slotIndex
    ...
    [MKCSMQTTInterface cs_configBXPDAdvParamsWithChannel:slotIndex interval:[interval integerValue] txPower:txPower bleMac:self.bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCSDAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSDAdvParamsModel alloc] init];
    }
    return _dataModel;
}
