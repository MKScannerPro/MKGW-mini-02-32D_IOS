//
//  MKCSBXPTAccelerometerController.m
...
@interface MKCSBXPTAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCSBXPTAccelerometerController

- (void)dealloc {
    NSLog(@"MKCSBXPTAccelerometerController销毁");
...
- (void)cs_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCSMQTTInterface cs_bxpBXPTNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCSReceiveBXPTAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
