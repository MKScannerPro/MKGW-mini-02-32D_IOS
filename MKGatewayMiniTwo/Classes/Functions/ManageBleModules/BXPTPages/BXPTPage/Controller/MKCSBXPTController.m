...
@interface MKCSBXPTController ()<UITableViewDelegate,
...
@implementation MKCSBXPTController

- (void)dealloc {
    NSLog(@"MKCSBXPTController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Remote reminder
        MKCSBXPTRemoteReminderController *vc = [[MKCSBXPTRemoteReminderController alloc] init];
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Motion event data
        MKCSBXPTMotionEventController *vc = [[MKCSBXPTMotionEventController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 2) {
        //Accelerometer data
        MKCSBXPTAccelerometerController *vc = [[MKCSBXPTAccelerometerController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 3) {
        ...
        MKCSBXPTAccParamsController *vc = [[MKCSBXPTAccParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 4) {
        ...
        MKCSBXPTAdvParamsController *vc = [[MKCSBXPTAdvParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)readDatasFromDevice {
    ...
    [MKCSMQTTInterface cs_readBXPTConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_bxpBXPTPowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
