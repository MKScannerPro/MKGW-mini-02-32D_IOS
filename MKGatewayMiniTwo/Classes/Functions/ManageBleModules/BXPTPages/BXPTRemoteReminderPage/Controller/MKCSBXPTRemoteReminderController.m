//
//  MKCSBXPTRemoteReminderController.m
...
@interface MKCSBXPTRemoteReminderController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBXPTRemoteReminderModel *dataModel;

@end

@implementation MKCSBXPTRemoteReminderController

- (void)dealloc {
    NSLog(@"MKCSBXPTRemoteReminderController销毁");
}
...
- (void)reminderLED {
    ...
    [MKCSMQTTInterface cs_bxpBXPTLedRemoteReminderWithBleMac:self.bleMac color:self.dataModel.color blinkingTime:[self.dataModel.blinkingTime integerValue] blinkingInterval:[self.dataModel.blinkingInterval integerValue] macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCSBXPTRemoteReminderModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBXPTRemoteReminderModel alloc] init];
    }
    return _dataModel;
}
