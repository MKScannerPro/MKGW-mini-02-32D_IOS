...
@interface MKCSBXPTAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSTAdvParamsModel *dataModel;

@end

@implementation MKCSBXPTAdvParamsController

- (void)dealloc {
    NSLog(@"MKCSBXPTAdvParamsController销毁");
}
...
- (void)cs_BXPAdvParamsCell_setPressedWithSlotIndex:(NSInteger)slotIndex
    ...
    [MKCSMQTTInterface cs_configBXPTAdvParamsWithChannel:slotIndex interval:[interval integerValue] txPower:txPower bleMac:self.bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKCSBXPAdvParamsCellSlotType)fetchSlotType:(NSInteger)slotType {
    ...
    if (slotType == 0x80) {
        return MKCSBXPAdvParamsCellSlotTypeTag;
    }
    if (slotType == 0xff) {
...
- (MKCSTAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSTAdvParamsModel alloc] init];
    }
    return _dataModel;
}
