//
//  MKCSBXPCHistoricalTHDataHeaderView.m
...
@interface MKCSBXPCHistoricalTHDataHeaderView ()
...
@implementation MKCSBXPCHistoricalTHDataHeaderView
...
- (void)syncButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_bxpcHistoricalHTDataHeaderView_syncButtonPressed:)]) {
        [self.delegate cs_bxpcHistoricalHTDataHeaderView_syncButtonPressed:!self.syncButton.selected];
    }
}

- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_bxpcHistoricalHTDataHeaderView_deleteButtonPressed)]) {
        [self.delegate cs_bxpcHistoricalHTDataHeaderView_deleteButtonPressed];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_bxpcHistoricalHTDataHeaderView_exportButtonPressed)]) {
        [self.delegate cs_bxpcHistoricalHTDataHeaderView_exportButtonPressed];
    }
}
...
- (UIImageView *)synIcon {
    if (!_synIcon) {
        ...
        _synIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPCHistoricalTHDataHeaderView", @"cs_threeAxisAcceLoadingIcon.png");
    }
    return _synIcon;
}
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPCHistoricalTHDataHeaderView", @"cs_slotExportEnableIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)deleteButton {
    if (!_deleteButton) {
        ...
        [_deleteButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPCHistoricalTHDataHeaderView", @"cs_slotExportDeleteIcon.png") forState:UIControlStateNormal];
