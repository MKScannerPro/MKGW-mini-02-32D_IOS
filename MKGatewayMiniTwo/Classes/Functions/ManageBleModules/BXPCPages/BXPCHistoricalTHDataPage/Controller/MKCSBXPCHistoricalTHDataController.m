//
//  MKCSBXPCHistoricalTHDataController.m
...
@interface MKCSBXPCHistoricalTHDataController ()<MFMailComposeViewControllerDelegate,
MKCSBXPCHistoricalTHDataHeaderViewDelegate>

@property (nonatomic, strong)MKCSBXPCHistoricalTHDataHeaderView *headerView;
...
@implementation MKCSBXPCHistoricalTHDataController

- (void)dealloc {
    NSLog(@"MKCSBXPCHistoricalTHDataController销毁");
...
#pragma mark - MKCSBXPCHistoricalTHDataHeaderViewDelegate
- (void)cs_bxpcHistoricalHTDataHeaderView_syncButtonPressed:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_bxpBXPCNotifyHistoricalHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCSReceiveBXPCHistoricalHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
...
- (void)cs_bxpcHistoricalHTDataHeaderView_deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_bxpBXPCDeleteHistoricalHTDataWithBleMac:self.bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
        self.textView.text = @"";
    } failedBlock:^(NSError * _Nonnull error) {
...
#pragma mark - getter
- (MKCSBXPCHistoricalTHDataHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKCSBXPCHistoricalTHDataHeaderView alloc] init];
