//
//  MKCSBXPCRealTimeTHDataController.m
...
@interface MKCSBXPCRealTimeTHDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCSBXPCRealTimeTHDataController

- (void)dealloc {
    NSLog(@"MKCSBXPCRealTimeTHDataController销毁");
...
- (void)cs_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCSMQTTInterface cs_bxpBXPCNotifyRealTimeHTDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveHTDatas:)
                                                         name:MKCSReceiveBXPCRealTimeHTDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
