//
//  MKCSBXPCAccelerometerController.m
...
@interface MKCSBXPCAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKCSBXPCAccelerometerController

- (void)dealloc {
    NSLog(@"MKCSBXPCAccelerometerController销毁");
...
- (void)cs_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKCSMQTTInterface cs_bxpBXPCNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKCSReceiveBXPCAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
