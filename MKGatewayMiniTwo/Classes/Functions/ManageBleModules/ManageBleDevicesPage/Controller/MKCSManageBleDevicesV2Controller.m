//
//  MKCSManageBleDevicesV2Controller.m
...
@interface MKCSManageBleDevicesV2Controller ()<UITableViewDelegate,
UITableViewDataSource,
MKCSManageBleDevicesSearchButtonDelegate,
MKCSManageBleDevicesCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)MKCSManageBleDevicesSearchButtonModel *buttonModel;

@property (nonatomic, strong)MKCSManageBleDevicesSearchButton *searchButton;
...
@implementation MKCSManageBleDevicesV2Controller

- (void)dealloc {
    NSLog(@"MKCSManageBleDevicesV2Controller销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCSManageBleDevicesCell *cell = [MKCSManageBleDevicesCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKCSManageBleDevicesSearchButtonDelegate
- (void)cs_scanSearchButtonMethod {
    [MKCSManageBleDeviceSearchView showSearchName:self.buttonModel.searchName
...
#pragma mark - MKCSManageBleDevicesCellDelegate
/// 用户点击了链接按钮
...
- (void)cs_manageBleDevicesCellConnectButtonPressed:(NSString *)macAddress typeCode:(NSInteger)typeCode {
    ...
    NSString *recordSelectedType = [[NSUserDefaults standardUserDefaults] objectForKey:@"cs_bleDeviceConnectType"];
    if (!ValidStr(recordSelectedType)) {
        [[NSUserDefaults standardUserDefaults] setObject:@"0" forKey:@"cs_bleDeviceConnectType"];
        recordSelectedType = @"1";
    }
    
    [MKCSManageBleDevicesTypeSelectedView showWithType:[recordSelectedType integerValue] selecteBlock:^(MKCSManageBleDevicesTypeSelectedViewType selectedType) {
        [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%ld",(long)selectedType] forKey:@"cs_bleDeviceConnectType"];
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKCSReceiveDeviceDatasNotification
                                               object:nil];
}
...
- (void)updateCellModelWithMac:(NSString *)macAddress
    ...
    MKCSManageBleDevicesCellModel *cellModel = [[MKCSManageBleDevicesCellModel alloc] init];
...
- (void)connectDeviceWithMacAddress:(NSString *)macAddress
                       selectedType:(MKCSManageBleDevicesTypeSelectedViewType)selectedType {
    if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeOther) {
    ...
    MKAlertViewAction *confirmAction = [[MKAlertViewAction alloc] initWithTitle:@"OK" handler:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeBXPBD) {
                //BXP-B-D
                [self connectBXPButtonWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeBXPBCR) {
                //BXP-B-CR
                [self connectBXPButtonCRWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeBXPC) {
                //BXP-C
                [self connectBXPCWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeBXPD) {
                //BXP-D
                [self connectBXPDWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeBXPT) {
                //BXP-T
                [self connectBXPTWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeBXPS) {
                //BXP-S
                [self connectBXPSWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypePIR) {
                //MK Pir
                [self connectMKPirWithPassword:self.asciiText bleMac:macAddress];
                return;
            }
            if (selectedType == MKCSManageBleDevicesTypeSelectedViewTypeTOF) {
                //MK Tof
    ...
    [alertView showAlertWithTitle:@"Enter password" message:@"" notificationName:@"mk_cs_needDismissAlert"];
}

- (void)connectBXPButtonWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectBXPButtonWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)connectBXPButtonCRWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectBXPButtonCRWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPCWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectBXPCWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPDWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectBXPDWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPTWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectBXPTWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectBXPSWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectBXPSWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectMKPirWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectMKPirWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectMKTofWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectMKTofWithPassword:password bleMac:bleMac macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (void)connectNormalDeviceWithBleMac:(NSString *)macAddress {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKCSMQTTInterface cs_connectNormalBleDeviceWithBleMac:macAddress macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (MKCSManageBleDevicesSearchButton *)searchButton {
    if (!_searchButton) {
        _searchButton = [[MKCSManageBleDevicesSearchButton alloc] init];
        _searchButton.delegate = self;
    }
    return _searchButton;
}

- (MKCSManageBleDevicesSearchButtonModel *)buttonModel {
    if (!_buttonModel) {
        _buttonModel = [[MKCSManageBleDevicesSearchButtonModel alloc] init];
