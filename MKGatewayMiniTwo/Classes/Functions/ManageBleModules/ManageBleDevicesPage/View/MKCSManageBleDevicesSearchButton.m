//
//  MKCSManageBleDevicesSearchButton.m
...
@implementation MKCSManageBleDevicesSearchButtonModel
@end

@interface MKCSManageBleDevicesSearchButton ()
...
@implementation MKCSManageBleDevicesSearchButton
...
- (void)clearButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(cs_scanSearchButtonClearMethod)]) {
        [self.delegate cs_scanSearchButtonClearMethod];
    }
}

- (void)searchButtonPressed{
    if ([self.delegate respondsToSelector:@selector(cs_scanSearchButtonMethod)]) {
        [self.delegate cs_scanSearchButtonMethod];
    }
}

#pragma mark - setting
- (void)setDataModel:(MKCSManageBleDevicesSearchButtonModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSManageBleDevicesSearchButtonModel.class]) {
        return;
    }
...
- (UIImageView *)searchIcon{
    if (!_searchIcon) {
        ...
        _searchIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSManageBleDevicesSearchButton", @"cs_searchGrayIcon.png");
    }
    return _searchIcon;
}
...
- (UIButton *)clearButton{
    if (!_clearButton) {
        ...
        [_clearButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSManageBleDevicesSearchButton", @"cs_clearButtonIcon.png") forState:UIControlStateNormal];
