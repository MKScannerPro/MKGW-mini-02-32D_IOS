...
@implementation MKCSManageBleDevicesCellModel
@end

@interface MKCSManageBleDevicesCell ()
...
@implementation MKCSManageBleDevicesCell

+ (MKCSManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKCSManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate cs_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCSManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSManageBleDevicesCellModel.class]) {
        return;
    }
