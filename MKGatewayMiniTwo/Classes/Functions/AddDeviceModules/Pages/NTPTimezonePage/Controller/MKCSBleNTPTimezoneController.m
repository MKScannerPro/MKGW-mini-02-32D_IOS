//
//  MKCSBleNTPTimezoneController.m
...
@interface MKCSBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKCSBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKCSBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBleNTPTimezoneController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
