...
@implementation MKCSMQTTLWTForDeviceViewModel
@end

@interface MKCSMQTTLWTForDeviceView ()
...
@implementation MKCSMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTLWTForDeviceView", @"cs_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTLWTForDeviceView", @"cs_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(cs_lwt_statusChanged:)]) {
        [self.delegate cs_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTLWTForDeviceView", @"cs_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTLWTForDeviceView", @"cs_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(cs_lwt_retainChanged:)]) {
        [self.delegate cs_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(cs_lwt_qosChanged:)]) {
        [self.delegate cs_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCSMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTLWTForDeviceView", @"cs_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTLWTForDeviceView", @"cs_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cs_lwt_topicChanged:)]) {
                [self.delegate cs_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cs_lwt_payloadChanged:)]) {
                [self.delegate cs_lwt_payloadChanged:text];
            }
        };
