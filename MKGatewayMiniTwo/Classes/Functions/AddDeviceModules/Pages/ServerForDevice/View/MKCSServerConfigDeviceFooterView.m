//
//  MKCSServerConfigDeviceFooterView.m
...
@implementation MKCSServerConfigDeviceFooterViewModel
@end
...
@interface MKCSServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCSMQTTSSLForDeviceViewDelegate,
MKCSServerConfigDeviceSettingViewDelegate,
MKCSMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKCSMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKCSMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKCSServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCSServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate cs_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate cs_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cs_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cs_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cs_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKCSMQTTSSLForDeviceViewDelegate
- (void)cs_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate cs_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)cs_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate cs_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)cs_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate cs_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)cs_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate cs_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)cs_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate cs_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKCSServerConfigDeviceSettingViewDelegate

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)cs_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate cs_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKCSMQTTLWTForDeviceViewDelegate
- (void)cs_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate cs_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)cs_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate cs_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)cs_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate cs_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)cs_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cs_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)cs_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cs_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKCSServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKCSMQTTSSLForDeviceViewModel *sslModel = [[MKCSMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKCSMQTTLWTForDeviceViewModel *lwtModel = [[MKCSMQTTLWTForDeviceViewModel alloc] init];
...
- (MKCSMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCSMQTTSSLForDeviceView alloc] init];
...
- (MKCSMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKCSMQTTLWTForDeviceView alloc] init];
...
- (MKCSServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKCSServerConfigDeviceSettingView alloc] init];
