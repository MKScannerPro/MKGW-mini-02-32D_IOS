...
@implementation MKCSMQTTSSLForDeviceViewModel
@end

@interface MKCSMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKCSMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate cs_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate cs_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate cs_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate cs_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate cs_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCSMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTSSLForDeviceView", @"cs_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTSSLForDeviceView", @"cs_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSMQTTSSLForDeviceView", @"cs_switchUnselectedIcon.png") forState:UIControlStateNormal];
