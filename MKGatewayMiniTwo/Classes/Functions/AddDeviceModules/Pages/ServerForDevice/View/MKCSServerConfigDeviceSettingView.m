//
//  MKCSServerConfigDeviceSettingView.m
...
@interface MKCSServerConfigDeviceSettingView ()
...
@implementation MKCSServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cs_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cs_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate cs_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
