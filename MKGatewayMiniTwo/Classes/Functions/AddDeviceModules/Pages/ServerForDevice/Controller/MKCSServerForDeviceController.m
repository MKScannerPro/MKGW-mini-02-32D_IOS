//
//  MKCSServerForDeviceController.m
...
@interface MKCSServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCSServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKCSImportServerControllerDelegate>
...
@property (nonatomic, strong)MKCSServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKCSServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKCSServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCSServerForDeviceController

- (void)dealloc {
    NSLog(@"MKCSServerForDeviceController销毁");
}
...
#pragma mark - MKCSServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain
- (void)cs_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)cs_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)cs_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)cs_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)cs_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)cs_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
    ...
    if (index == 1) {
        //Import Config File
        MKCSImportServerController *vc = [[MKCSImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (index == 2) {
...
#pragma mark - MKCSImportServerControllerDelegate
- (void)cs_selectedServerParams:(NSString *)fileName {
    [MKCSExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKCSServerForDeviceModel *model = [MKCSServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCSDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCSDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKCSDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKCSDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKCSDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKCSDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKCSDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKCSDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
//    }
    ...
    [MKCSExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)clearAllParams {
    MKCSServerForDeviceModel *emptyModel = [[MKCSServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSServerForDeviceController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCSServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCSServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCSServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCSServerConfigDeviceFooterViewModel alloc] init];
    }
