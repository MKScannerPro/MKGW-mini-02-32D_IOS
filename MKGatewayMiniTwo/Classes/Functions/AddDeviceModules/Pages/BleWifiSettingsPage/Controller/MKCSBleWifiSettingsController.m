//
//  MKCSBleWifiSettingsController.m
...
@interface MKCSBleWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBleWifiSettingsModel *dataModel;

@end

@implementation MKCSBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCSBleWifiSettingsController销毁");
}
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKCSBleWifiSettingsCertCellModel *cellModel = self.section8List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKCSBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKCSBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKCSDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKCSDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 8) {
        //CA certificate.
        MKCSBleWifiSettingsCertCell *cell = [MKCSBleWifiSettingsCertCell initCellWithTableView:self.tableView];
...
- (void)loadSection8Datas {
    MKCSBleWifiSettingsCertCellModel *cellModel = [[MKCSBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBleWifiSettingsController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
