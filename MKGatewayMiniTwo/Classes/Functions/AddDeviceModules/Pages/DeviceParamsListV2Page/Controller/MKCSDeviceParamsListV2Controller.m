//
//  MKCSDeviceParamsListV2Controller.m
...
@interface MKCSDeviceParamsListV2Controller ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKCSDeviceParamsListV2Controller

- (void)dealloc {
    NSLog(@"MKCSDeviceParamsListV2Controller销毁");
...
- (void)viewDidLoad {
    ...
    [MKCSDeviceMQTTParamsModel shared].deviceModel.deviceType = @"71";
...
- (void)leftButtonMethod {
    ...
    [[MKCSCentralManager shared] disconnect];
    [MKCSDeviceMQTTParamsModel sharedDealloc];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Network Settings
        MKCSBleWifiSettingsController *vc = [[MKCSBleWifiSettingsController alloc] init];
        vc.isV2 = YES;
    ...
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT settings
        MKCSServerForDeviceController *vc = [[MKCSServerForDeviceController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 1 && indexPath.row == 0) {
        //NTP & Timezone
        MKCSBleNTPTimezoneController *vc = [[MKCSBleNTPTimezoneController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //Scanner Filter
        MKCSBleScannerFilterController *vc = [[MKCSBleScannerFilterController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Advertise iBeacon
        MKCSBleAdvBeaconV2Controller *vc = [[MKCSBleAdvBeaconV2Controller alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 3) {
        //Device Information
        MKCSBleDeviceInfoV2Controller *vc = [[MKCSBleDeviceInfoV2Controller alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)receiveDeviceOnline:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"macAddress"]) || ![[MKCSDeviceMQTTParamsModel shared].deviceModel.macAddress isEqualToString:user[@"macAddress"]]) {
        return;
    }
...
- (void)connectButtonPressed {
    ...
    if (![MKCSDeviceMQTTParamsModel shared].wifiConfig || ![MKCSDeviceMQTTParamsModel shared].mqttConfig) {
        [self.view showCentralToast:@"Please configure WIFI and MQTT settings first!"];
        return;
    }
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKCSInterface cs_enterSTAModeWithSucBlock:^{
...
- (void)startMqttProcess {
    ...
    if (ValidStr([MKCSMQTTDataManager shared].serverParams.subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKCSMQTTDataManager shared].serverParams.subscribeTopic;
    }else {
        topic = [MKCSDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    }
    [self.progressView show];
    [[MKCSMQTTDataManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    MKCSDeviceModel *deviceModel = [[MKCSDeviceModel alloc] init];
    deviceModel.deviceType = [MKCSDeviceMQTTParamsModel shared].deviceModel.deviceType;
    deviceModel.clientID = [MKCSDeviceMQTTParamsModel shared].deviceModel.clientID;
    deviceModel.subscribedTopic = [MKCSDeviceMQTTParamsModel shared].deviceModel.subscribedTopic;
    deviceModel.publishedTopic = [MKCSDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    deviceModel.macAddress = [MKCSDeviceMQTTParamsModel shared].deviceModel.macAddress;
    deviceModel.deviceName = [MKCSDeviceMQTTParamsModel shared].deviceModel.deviceName;
    deviceModel.lwtStatus = [MKCSDeviceMQTTParamsModel shared].deviceModel.lwtStatus;
    deviceModel.lwtTopic = [MKCSDeviceMQTTParamsModel shared].deviceModel.lwtTopic;
        
    MKCSConnectSuccessController *vc = [[MKCSConnectSuccessController alloc] init];
