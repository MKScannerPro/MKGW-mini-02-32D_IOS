...
@implementation MKCSNearbyWifiCellModel
@end

@interface MKCSNearbyWifiCell ()
...
@implementation MKCSNearbyWifiCell


+ (MKCSNearbyWifiCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSNearbyWifiCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSNearbyWifiCellIdenty"];
    if (!cell) {
        cell = [[MKCSNearbyWifiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSNearbyWifiCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKCSNearbyWifiCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSNearbyWifiCellModel.class]) {
        return;
    }
    //顶部
