...
@interface MKCSNearbyWifiController ()<UITableViewDelegate,
UITableViewDataSource,
mk_cs_centralManagerScanWifiDelegate>
...
@implementation MKCSNearbyWifiController

- (void)dealloc {
    NSLog(@"MKCSNearbyWifiController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCSNearbyWifiCellModel *cellModel = self.dataList[indexPath.row];
    if ([self.delegate respondsToSelector:@selector(cs_nearbyWifiController_selectedWifi:)]) {
        [self.delegate cs_nearbyWifiController_selectedWifi:cellModel.ssid];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKCSNearbyWifiCell *cell = [MKCSNearbyWifiCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_cs_centralManagerScanWifiDelegate
- (void)mk_cs_receiveWifi:(NSString *)content {
...
- (void)startScanWifi {
    ...
    @weakify(self);
    [MKCSInterface cs_startWifiScanWithSucBlock:^{
        [MKCSCentralManager shared].wifiDelegate = self;
...
- (void)parseContentList {
    ...
    while (index < length) {
        ...
        // 创建模型对象
        MKCSNearbyWifiCellModel *model = [[MKCSNearbyWifiCellModel alloc] init];
        model.bssid = bssid;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSNearbyWifiController", @"cs_refreshWifiListIcon.png") forState:UIControlStateNormal];
