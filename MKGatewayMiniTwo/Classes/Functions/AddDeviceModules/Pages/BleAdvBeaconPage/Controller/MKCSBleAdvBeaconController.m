...
@interface MKCSBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBleAdvBeaconModel *dataModel;

@end

@implementation MKCSBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKCSBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBleAdvBeaconController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
