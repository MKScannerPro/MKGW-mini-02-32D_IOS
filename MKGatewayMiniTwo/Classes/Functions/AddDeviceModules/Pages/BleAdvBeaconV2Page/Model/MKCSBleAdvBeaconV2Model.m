...
@interface MKCSBleAdvBeaconV2Model ()
...
@implementation MKCSBleAdvBeaconV2Model
...
- (BOOL)readConnectable {
    __block BOOL success = NO;
    [MKCSInterface cs_readConnectableWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configConnectable {
    __block BOOL success = NO;
    [MKCSInterface cs_configConnectable:self.connectable sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
