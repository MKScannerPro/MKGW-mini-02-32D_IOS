//
//  MKCSBleScannerFilterController.m
...
@interface MKCSBleScannerFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSBleScannerFilterModel *dataModel;

@end

@implementation MKCSBleScannerFilterController

- (void)dealloc {
    NSLog(@"MKCSBleScannerFilterController销毁");
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 2) {
        return ([[MKCSDeviceMQTTParamsModel shared].deviceModel.deviceType isEqualToString:@"71"] ? self.section2List.count : 0);
    }
    return 0;
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = ([[MKCSDeviceMQTTParamsModel shared].deviceModel.deviceType isEqualToString:@"71"] ? @"Scan & Upload" : @"Scanner Filter");
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBleScannerFilterController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSBleScannerFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSBleScannerFilterModel alloc] init];
    }
    return _dataModel;
}
