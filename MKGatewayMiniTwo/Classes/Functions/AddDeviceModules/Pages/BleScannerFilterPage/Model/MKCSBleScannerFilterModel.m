...
@interface MKCSBleScannerFilterModel ()
...
@implementation MKCSBleScannerFilterModel
...
- (void)readDataWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        ...
        if ([[MKCSDeviceMQTTParamsModel shared].deviceModel.deviceType isEqualToString:@"71"]) {
...
- (void)configDataWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        ...
        if ([[MKCSDeviceMQTTParamsModel shared].deviceModel.deviceType isEqualToString:@"70"]) {
            //V1支持
...
- (BOOL)readFilterRssi {
    ...
    [MKCSInterface cs_readRssiFilterValueWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterRssi {
    __block BOOL success = NO;
    [MKCSInterface cs_configRssiFilterValue:self.rssi sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readMacAddress {
    __block BOOL success = NO;
    [MKCSInterface cs_readFilterMACAddressListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMacAddress {
    ...
    [MKCSInterface cs_configFilterMACAddressList:macList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)readAdvName {
    __block BOOL success = NO;
    [MKCSInterface cs_readFilterAdvNameListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvName {
    ...
    [MKCSInterface cs_configFilterAdvNameList:nameList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)configRelationShip {
    __block BOOL success = NO;
    [MKCSInterface cs_configFilterRelationship:7
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)readFilterInterval {
    __block BOOL success = NO;
    [MKCSInterface cs_readFilterReportIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterInterval {
    __block BOOL success = NO;
    [MKCSInterface cs_configFilterReportInterval:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
