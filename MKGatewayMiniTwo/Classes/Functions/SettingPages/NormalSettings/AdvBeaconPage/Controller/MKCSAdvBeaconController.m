...
@interface MKCSAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSAdvBeaconModel *dataModel;

@end

@implementation MKCSAdvBeaconController

- (void)dealloc {
    NSLog(@"MKCSAdvBeaconController销毁");
}
...
- (MKCSAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
