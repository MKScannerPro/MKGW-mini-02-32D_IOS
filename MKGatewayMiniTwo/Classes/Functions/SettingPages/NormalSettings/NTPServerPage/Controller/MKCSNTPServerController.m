...
@interface MKCSNTPServerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSNTPServerModel *dataModel;

@end

@implementation MKCSNTPServerController

- (void)dealloc {
    NSLog(@"MKCSNTPServerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSNTPServerController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSNTPServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSNTPServerModel alloc] init];
    }
    return _dataModel;
}
