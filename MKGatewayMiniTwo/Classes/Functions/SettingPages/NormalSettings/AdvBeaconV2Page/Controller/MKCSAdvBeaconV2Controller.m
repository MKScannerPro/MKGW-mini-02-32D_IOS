...
@interface MKCSAdvBeaconV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSAdvBeaconV2Model *dataModel;

@end

@implementation MKCSAdvBeaconV2Controller

- (void)dealloc {
    NSLog(@"MKCSAdvBeaconV2Controller销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSAdvBeaconV2Controller", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSAdvBeaconV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSAdvBeaconV2Model alloc] init];
    }
    return _dataModel;
}
