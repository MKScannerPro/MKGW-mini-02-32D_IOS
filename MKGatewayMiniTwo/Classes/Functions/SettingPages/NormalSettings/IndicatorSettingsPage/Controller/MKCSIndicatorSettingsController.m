//
//  MKCSIndicatorSettingsController.m
...
@interface MKCSIndicatorSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSIndicatorSetingsModel *dataModel;

@end

@implementation MKCSIndicatorSettingsController

- (void)dealloc {
    NSLog(@"MKCSIndicatorSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSIndicatorSettingsController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSIndicatorSetingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSIndicatorSetingsModel alloc] init];
    }
    return _dataModel;
}
