...
@interface MKCSSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKCSSystemTimeCellDelegate,
...
@implementation MKCSSystemTimeController

- (void)dealloc {
    NSLog(@"MKCSSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKCSNTPServerController *vc = [[MKCSNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCSSystemTimeCell *cell = [MKCSSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKCSSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKCSMQTTInterface cs_readDeviceUTCTimeWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKCSMQTTInterface cs_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKCSSystemTimeCellModel *cellModel = [[MKCSSystemTimeCellModel alloc] init];
    cellModel.index = 0;
