//
//  MKCSMqttServerConfigFooterView.m
...
@implementation MKCSMqttServerConfigFooterViewModel
@end
...
@interface MKCSMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKCSMqttServerSSLViewDelegate,
MKCSMqttServerSettingViewDelegate,
MKCSMqttServerLwtViewDelegate>
...
@property (nonatomic, strong)MKCSMqttServerSSLView *sslView;

@property (nonatomic, strong)MKCSMqttServerLwtView *lwtView;

@property (nonatomic, strong)MKCSMqttServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKCSMqttServerConfigFooterView
...
#pragma mark - MKCSMqttServerSSLViewDelegate
...
- (void)cs_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate cs_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}

#pragma mark - MKCSMqttServerSettingViewDelegate
...
#pragma mark - MKCSMqttServerLwtViewDelegate
...
#pragma mark - setter
- (void)setDataModel:(MKCSMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKCSMqttServerSSLViewModel *sslModel = [[MKCSMqttServerSSLViewModel alloc] init];
    ...
    MKCSMqttServerLwtViewModel *lwtModel = [[MKCSMqttServerLwtViewModel alloc] init];
...
- (MKCSMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKCSMqttServerSSLView alloc] init];
...
- (MKCSMqttServerLwtView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKCSMqttServerLwtView alloc] init];
...
- (MKCSMqttServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKCSMqttServerSettingView alloc] init];
