...
@implementation MKCSMqttServerSSLViewModel
@end

@interface MKCSMqttServerSSLView ()<MKCSMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKCSMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKCSMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKCSMqttServerSSLTextField *clientCertView;

@end

@implementation MKCSMqttServerSSLView
...
#pragma mark - MKCSMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)cs_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate cs_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate cs_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cs_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate cs_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCSMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKCSMqttServerSSLTextFieldModel *caModel = [[MKCSMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKCSMqttServerSSLTextFieldModel *clientKeyModel = [[MKCSMqttServerSSLTextFieldModel alloc] init];
    ...
    MKCSMqttServerSSLTextFieldModel *clientModel = [[MKCSMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayMiniTwo", @"MKCSMqttServerSSLView", @"cs_switchSelectedIcon.png") : LOADICON(@"MKGatewayMiniTwo", @"MKCSMqttServerSSLView", @"cs_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSMqttServerSSLView", @"cs_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKCSMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKCSMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKCSMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKCSMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKCSMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKCSMqttServerSSLTextField alloc] init];
