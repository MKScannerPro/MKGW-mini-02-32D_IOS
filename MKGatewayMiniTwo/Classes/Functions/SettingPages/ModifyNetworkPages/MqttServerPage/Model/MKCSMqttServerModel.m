...
@implementation MKCSMqttServerModel
...
- (void)updateValue:(MKCSMqttServerModel *)model {
    if (!model || ![model isKindOfClass:MKCSMqttServerModel.class]) {
        return;
    }
...
- (BOOL)readMqttInfos {
    ...
    [MKCSMQTTInterface cs_readMQTTParamsWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMqttInfos {
    ...
    if ([self.lwtTopic isEqualToString:defaultPubTopic]) {
        //用户使用默认的LWT topic
        self.lwtTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKCSDeviceModeManager shared].deviceName,self.clientID,@"device_to_app"];
    }
    if ([self.publishTopic isEqualToString:defaultPubTopic]) {
        //用户使用默认的topic
        self.publishTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKCSDeviceModeManager shared].deviceName,self.clientID,@"device_to_app"];
    }
    if ([self.subscribeTopic isEqualToString:defaultSubTopic]) {
        //用户使用默认的topic
        self.subscribeTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKCSDeviceModeManager shared].deviceName,self.clientID,@"app_to_device"];
    }
    if (!self.sslIsOn) {
        self.connectMode = 0;
    }else {
    ...
    [MKCSMQTTInterface cs_modifyMqttInfos:self macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configMqttCerts {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_modifyMqttCerts:self macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
