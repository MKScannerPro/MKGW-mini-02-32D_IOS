...
@interface MKCSMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKCSMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKCSMqttServerModel *dataModel;

@property (nonatomic, strong)MKCSMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKCSMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKCSMqttServerController

- (void)dealloc {
    NSLog(@"MKCSMqttServerController销毁");
}
...
#pragma mark - MKCSMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)cs_selectedServerParams:(NSString *)fileName {
    [MKCSExcelDataManager parseDeviceExcel:fileName
        ...
        MKCSMqttServerModel *model = [MKCSMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKCSMqttServerModel *model = [[MKCSMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSMqttServerController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKCSMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKCSMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKCSMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKCSMqttServerConfigFooterViewModel alloc] init];
    }
