//
//  MKCSMqttWifiSettingsController.m
...
@interface MKCSMqttWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSMqttWifiSettingsModel *dataModel;

@end

@implementation MKCSMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKCSMqttWifiSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSMqttWifiSettingsController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
