...
@interface MKCSMqttWifiSettingsModel ()
...
@implementation MKCSMqttWifiSettingsModel
...
- (BOOL)readWifiInfos {
    ...
    [MKCSMQTTInterface cs_readWifiInfosWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configWifiInfos {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_modifyWifiInfos:self macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNetworkInfos {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_readNetworkInfosWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkInfos {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_modifyNetworkInfos:self macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
