//
//  MKCSMqttWifiSettingsBandCell.m
...
#import "MKCSMqttWifiSettingsBandCell.h"
...
@implementation MKCSMqttWifiSettingsBandCellModel
@end

@interface MKCSMqttWifiSettingsBandCell ()
...
@implementation MKCSMqttWifiSettingsBandCell

+ (MKCSMqttWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSMqttWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSMqttWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKCSMqttWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSMqttWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cs_mqttWifiSettingsBandCell_countryChanged:)]) {
            [self.delegate cs_mqttWifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCSMqttWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSMqttWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
