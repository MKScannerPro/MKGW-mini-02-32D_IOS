...
@interface MKCSOTAController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSOTAPageModel *dataModel;

@end

@implementation MKCSOTAController

- (void)dealloc {
    NSLog(@"MKCSOTAController销毁");
        ...
        if (self.dataModel.otaType == 0) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKCSReceiveDeviceOTAResultNotification
                                                       object:nil];
        }else if (self.dataModel.otaType == 1) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKCSReceiveDeviceNpcOTAResultNotification
                                                       object:nil];
        }
...
- (MKCSOTAPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSOTAPageModel alloc] init];
    }
    return _dataModel;
}
