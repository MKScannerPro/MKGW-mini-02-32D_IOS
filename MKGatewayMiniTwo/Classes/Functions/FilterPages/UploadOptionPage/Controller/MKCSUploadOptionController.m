...
@interface MKCSUploadOptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSUploadOptionModel *dataModel;

@end

@implementation MKCSUploadOptionController
...
- (void)filterByRawData {
    MKCSFilterByRawDataController *vc = [[MKCSFilterByRawDataController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSUploadOptionController", @"cs_saveIcon.png")
...
- (MKCSUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSUploadOptionModel alloc] init];
    }
    return _dataModel;
}
