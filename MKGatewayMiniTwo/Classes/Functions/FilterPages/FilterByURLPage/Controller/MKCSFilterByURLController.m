...
@interface MKCSFilterByURLController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByURLModel *dataModel;

@end

@implementation MKCSFilterByURLController

- (void)dealloc {
    NSLog(@"MKCSFilterByURLController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByURLController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByURLModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByURLModel alloc] init];
    }
    return _dataModel;
}
