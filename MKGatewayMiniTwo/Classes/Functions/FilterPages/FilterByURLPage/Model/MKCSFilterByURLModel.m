...
@interface MKCSFilterByURLModel ()
...
@implementation MKCSFilterByURLModel
...
- (BOOL)readFilterByURL {
    ...
    [MKCSMQTTInterface cs_readFilterByUrlWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByURL {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configFilterByURL:self.isOn url:self.url macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
