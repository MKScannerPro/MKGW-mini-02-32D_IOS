...
@interface MKCSFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByMacModel *dataModel;

@end

@implementation MKCSFilterByMacController
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByMacController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByMacModel alloc] init];
    }
    return _dataModel;
}
