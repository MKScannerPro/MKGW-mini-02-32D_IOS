...
@implementation MKCSFilterRawAdvDataModel
...
@interface MKCSFilterByOtherModel ()
...
@implementation MKCSFilterByOtherModel
...
- (BOOL)readFilterByOther {
    ...
    [MKCSMQTTInterface cs_readFilterOtherDatasWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByOther:(NSArray <MKCSFilterRawAdvDataModel *>*)list relationship:(mk_cs_filterByOther)relationship {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configFilterByOtherDatas:self.isOn relationship:relationship rawDataList:list macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
