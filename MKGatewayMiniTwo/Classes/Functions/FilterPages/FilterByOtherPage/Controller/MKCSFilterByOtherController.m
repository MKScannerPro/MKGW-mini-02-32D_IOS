...
@interface MKCSFilterByOtherController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByOtherModel *dataModel;

@end

@implementation MKCSFilterByOtherController

- (void)dealloc {
    NSLog(@"MKCSFilterByOtherController销毁");
}
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        ...
        MKCSFilterRawAdvDataModel *model = [[MKCSFilterRawAdvDataModel alloc] init];
...
- (mk_cs_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_cs_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_cs_filterByOther_AB;
        }
        return mk_cs_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_cs_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_cs_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_cs_filterByOther_AOrBOrC;
        }
    }
    return mk_cs_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByOtherController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
