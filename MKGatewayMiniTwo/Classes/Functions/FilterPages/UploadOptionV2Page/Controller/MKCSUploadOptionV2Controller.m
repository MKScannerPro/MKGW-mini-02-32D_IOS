//
//  MKCSUploadOptionV2Controller.m
...
@interface MKCSUploadOptionV2Controller ()<UITableViewDelegate,
UITableViewDataSource,
MKNormalSliderCellDelegate,
MKCSFilterCellDelegate>
...
@property (nonatomic, strong)MKCSUploadOptionV2Model *dataModel;

@end

@implementation MKCSUploadOptionV2Controller
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKCSFilterCell *cell = [MKCSFilterCell initCellWithTableView:tableView];
...
- (void)cs_filterValueChanged:(NSInteger)dataListIndex index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKCSFilterCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
    if (index == 2) {
        ...
        MKCSFilterCellModel *cellModel = self.section3List[1];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)filterByMACAddress {
    MKCSFilterByMacController *vc = [[MKCSFilterByMacController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByADVName {
    MKCSFilterByAdvNameController *vc = [[MKCSFilterByAdvNameController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByRawData {
    MKCSFilterByRawDataV2Controller *vc = [[MKCSFilterByRawDataV2Controller alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)duplicateDataFilter {
    MKCSDuplicateDataFilterController *vc = [[MKCSDuplicateDataFilterController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataOption {
    MKCSUploadDataOptionController *vc = [[MKCSUploadDataOptionController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataInterval {
    MKCSDataUploadIntervalController *vc = [[MKCSDataUploadIntervalController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSection1Datas {
    MKCSFilterCellModel *cellModel = [[MKCSFilterCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSection3Datas {
    MKCSFilterCellModel *cellModel1 = [[MKCSFilterCellModel alloc] init];
    cellModel1.index = 1;
    ...
    MKCSFilterCellModel *cellModel2 = [[MKCSFilterCellModel alloc] init];
    cellModel2.index = 2;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSUploadOptionV2Controller", @"cs_saveIcon.png")
...
- (MKCSUploadOptionV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSUploadOptionV2Model alloc] init];
    }
    return _dataModel;
}
