...
@interface MKCSDataUploadIntervalModel ()
...
@implementation MKCSDataUploadIntervalModel
...
- (BOOL)readUploadInterval {
    ...
    [MKCSMQTTInterface cs_readUploadDataIntervalWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configUploadInterval {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configUploadDataInterval:[self.interval integerValue] macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
