//
//  MKCSFilterByAdvNameController.m
...
@interface MKCSFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByAdvNameModel *dataModel;

@end

@implementation MKCSFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKCSFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByAdvNameController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
