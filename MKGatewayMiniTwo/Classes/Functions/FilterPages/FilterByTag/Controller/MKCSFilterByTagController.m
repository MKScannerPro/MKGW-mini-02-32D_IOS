...
@interface MKCSFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByTagModel *dataModel;

@end

@implementation MKCSFilterByTagController

- (void)dealloc {
    NSLog(@"MKCSFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByTagController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByTagModel alloc] init];
    }
    return _dataModel;
}
