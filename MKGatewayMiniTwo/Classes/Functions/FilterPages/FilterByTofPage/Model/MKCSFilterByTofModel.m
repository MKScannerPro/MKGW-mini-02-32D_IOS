...
@interface MKCSFilterByTofModel ()
...
@implementation MKCSFilterByTofModel
...
- (BOOL)readFilterByTof {
    ...
    [MKCSMQTTInterface cs_readFilterByTofWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByTof:(NSArray <NSString *>*)codeList {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configFilterByTofList:codeList isOn:self.isOn macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
