...
@interface MKCSFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByTofModel *dataModel;

@end

@implementation MKCSFilterByTofController

- (void)dealloc {
    NSLog(@"MKCSFilterByTofController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByTofController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByTofModel alloc] init];
    }
    return _dataModel;
}
