...
@interface MKCSFilterByNanoBeaconModel ()
...
@implementation MKCSFilterByNanoBeaconModel
...
- (BOOL)readFilterInformation {
    ...
    [MKCSMQTTInterface cs_readFilterByNanoBeaconWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterInformation:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configFilterByNanoBeacon:self.isOn advType:self.triggerType manufactureIDList:list macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
