//
//  MKCSFilterByNanoBeaconController.m
...
@interface MKCSFilterByNanoBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByNanoBeaconModel *dataModel;

@end

@implementation MKCSFilterByNanoBeaconController

- (void)dealloc {
    NSLog(@"MKCSFilterByNanoBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByNanoBeaconController", @"cs_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByNanoBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByNanoBeaconModel alloc] init];
    }
    return _dataModel;
}
