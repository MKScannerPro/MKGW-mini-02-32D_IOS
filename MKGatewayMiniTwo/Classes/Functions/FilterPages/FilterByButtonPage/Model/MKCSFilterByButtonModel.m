...
@interface MKCSFilterByButtonModel ()
...
@implementation MKCSFilterByButtonModel
...
- (BOOL)readFilterByBXPButton {
    ...
    [MKCSMQTTInterface cs_readFilterBXPButtonWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByBXPButton {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configFilterBXPButton:self macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
