//
//  MKCSFilterByBeaconController.m
...
@interface MKCSFilterByBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByBeaconModel *dataModel;

@end

@implementation MKCSFilterByBeaconController

- (void)dealloc {
    NSLog(@"MKCSFilterByBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByBeaconController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByBeaconModel alloc] initWithPageType:self.pageType];
    }
    return _dataModel;
}
