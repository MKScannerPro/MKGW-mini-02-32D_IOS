...
@interface MKCSFilterByUIDController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByUIDModel *dataModel;

@end

@implementation MKCSFilterByUIDController

- (void)dealloc {
    NSLog(@"MKCSFilterByUIDController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByUIDController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByUIDModel alloc] init];
    }
    return _dataModel;
}
