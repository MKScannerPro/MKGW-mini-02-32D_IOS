...
@interface MKCSFilterByUIDModel ()
...
@implementation MKCSFilterByUIDModel
...
- (BOOL)readFilterByUID {
    ...
    [MKCSMQTTInterface cs_readFilterByUIDWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByUID {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configFilterByUID:self.isOn namespaceID:self.namespaceID instanceID:self.instanceID macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
