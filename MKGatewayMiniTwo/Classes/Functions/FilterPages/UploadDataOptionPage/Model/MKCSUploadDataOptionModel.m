...
@interface MKCSUploadDataOptionModel ()
...
@implementation MKCSUploadDataOptionModel
...
- (BOOL)readUploadDataOption {
    ...
    [MKCSMQTTInterface cs_readUploadDataOptionWithMacAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configUploadDataOption {
    __block BOOL success = NO;
    [MKCSMQTTInterface cs_configUploadDataOption:self macAddress:[MKCSDeviceModeManager shared].macAddress topic:[MKCSDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
