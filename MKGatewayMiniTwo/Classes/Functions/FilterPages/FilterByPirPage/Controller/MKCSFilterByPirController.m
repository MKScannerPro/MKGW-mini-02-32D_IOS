...
@interface MKCSFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCSFilterByPirModel *dataModel;

@end

@implementation MKCSFilterByPirController

- (void)dealloc {
    NSLog(@"MKCSFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSFilterByPirController", @"cs_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCSFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCSFilterByPirModel alloc] init];
    }
    return _dataModel;
}
