...
static NSString *const localPasswordKey = @"mk_cs_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKCSScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_cs_centralManagerScanDelegate>
...
@implementation MKCSScanPageController

- (void)dealloc {
    NSLog(@"MKCSScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKCSCentralManager shared] stopScan];
    [MKCSCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKCSCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKCSCentralManager shared].centralStatus != mk_cs_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKCSScanPageCell *cell = [MKCSScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_cs_centralManagerScanDelegate

- (void)mk_cs_receiveDevice:(NSDictionary *)deviceModel {
    MKCSScanPageModel *dataModel = [MKCSScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKCSCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKCSScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKCSScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKCSCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKCSScanPageModel *)deviecModel {
    ...
    [[MKCSCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
        [[NSUserDefaults standardUserDefaults] setObject:password forKey:localPasswordKey];
        [[MKHudManager share] hide];
        if ([deviecModel.deviceType isEqualToString:@"70"]) {
            //V1
            MKCSDeviceParamsListController *vc = [[MKCSDeviceParamsListController alloc] init];
            vc.deviceType = deviecModel.deviceType;
            [self.navigationController pushViewController:vc animated:YES];
            return;
        }
        if ([deviecModel.deviceType isEqualToString:@"71"]) {
            //V2
...
- (void)readDeviceMode:(NSString *)deviceType {
    ...
    [MKCSInterface cs_readDeviceModeWithSucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        self.rightButton.selected = NO;
        MKCSDeviceParamsListV2Controller *vc = [[MKCSDeviceParamsListV2Controller alloc] init];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSScanPageController", @"cs_scanRefresh.png");
    }
    return _refreshIcon;
}
