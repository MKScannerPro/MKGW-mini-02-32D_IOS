...
@implementation MKCSDeviceListCell

+ (MKCSDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKCSDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(cs_cellDeleteButtonPressed:)]) {
        [self.delegate cs_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCSDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    self.stateLabel.text = (_dataModel.onLineState == MKCSDeviceModelStateOnline ? @"Online" : @"Offline");
    self.stateLabel.textColor = (_dataModel.onLineState == MKCSDeviceModelStateOnline ? NAVBAR_COLOR_MACROS : UIColorFromRGB(0xcccccc));
    self.macLabel.text = SafeStr(_dataModel.macAddress);
    if (dataModel.onLineState == MKCSDeviceModelStateOffline) {
        //设备离线
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSDeviceListCell", @"cs_offline_wifi.png");
        return;
    }
    //设备在线
    if (_dataModel.wifiLevel >= -50) {
        //Good
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSDeviceListCell", @"cs_good_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel >= -65 && _dataModel.wifiLevel < -50) {
        //Medium
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSDeviceListCell", @"cs_medium_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel < -65) {
        //Poor
        self.wifiIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSDeviceListCell", @"cs_poor_wifi.png");
        return;
    }
}
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSDeviceListCell", @"cs_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSDeviceListCell", @"cs_goNextButton.png");
    }
    return _rightIcon;
}
