...
@implementation MKCSCreatScannerProDeviceModel
...
@interface MKCSNetworkService ()

@property (nonatomic, strong)NSURLSessionDataTask *addScannerProDeviceTask;

@end

@implementation MKCSNetworkService

+ (instancetype)share {
    static dispatch_once_t t;
    static MKCSNetworkService *service = nil;
    dispatch_once(&t, ^{
        service = [[MKCSNetworkService alloc] init];
    });
    return service;
}

- (void)addScannerProDevicesToCloud:(NSArray <MKCSCreatScannerProDeviceModel *>*)deviceList
                             isHome:(BOOL)isHome
    ...
    for (MKCSCreatScannerProDeviceModel *deviceModel in deviceList) {
