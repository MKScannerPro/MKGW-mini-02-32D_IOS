...
static MKCSMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKCSMQTTServerManager)

@end

@implementation NSObject (MKCSMQTTServerManager)
...
@interface MKCSMQTTServerManager ()

@property (nonatomic, assign)MKCSMQTTSessionManagerState state;

@property (nonatomic, strong)MKCSServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKCSServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKCSMQTTServerManager
...
+ (MKCSMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKCSMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKCSServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cs_didReceiveMessage:onTopic:)]) {
                [protocol cs_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKCSServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(cs_didChangeState:)]) {
                [protocol cs_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKCSMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKCSServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKCSServerManagerProtocol)]
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_cs_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cs_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_cs_mqtt_recordDate"];
...
#pragma mark - getter
- (MKCSServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKCSServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKCSServerManagerProtocol>> *)managerList {
    if (!_managerList) {
