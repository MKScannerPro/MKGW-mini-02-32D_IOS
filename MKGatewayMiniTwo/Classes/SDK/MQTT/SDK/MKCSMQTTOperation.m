...
/**
 线程ID
 */
@property (nonatomic, assign)mk_cs_serverOperationID operationID;
...
@implementation MKCSMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKCSMQTTOperation销毁");
}
...
#pragma mark - MKCSMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKCSMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_cs_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_cs_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
