...
typedef NS_ENUM(NSInteger, mk_cs_keyResetType) {
    mk_cs_keyResetType_powerOnOneMin,     //Press in 1 min after powered
    mk_cs_keyResetType_pressAnyTime,      //Press any time
};
...
typedef NS_ENUM(NSInteger, mk_cs_filterRelationship) {
    ...
    mk_cs_filterRelationship_advNameAndRawDataAndMac,
    mk_cs_filterRelationship_advNameOrRawData,
    mk_cs_filterRelationship_advNameAndRawMac,
};

typedef NS_ENUM(NSInteger, mk_cs_filterByTLM) {
    mk_cs_filterByTLM_nonEncrypted,        //Non-encrypted type TLM
    mk_cs_filterByTLM_encrypted,           //Encryption type TLM
    mk_cs_filterByTLM_all,                //Filter all Eddystone_TLM data
};

typedef NS_ENUM(NSInteger, mk_cs_filterByOther) {
    mk_cs_filterByOther_A,                 //Filter by A condition.
    mk_cs_filterByOther_AB,                //Filter by A & B condition.
    mk_cs_filterByOther_AOrB,              //Filter by A | B condition.
    mk_cs_filterByOther_ABC,               //Filter by A & B & C condition.
    mk_cs_filterByOther_ABOrC,             //Filter by (A & B) | C condition.
    mk_cs_filterByOther_AOrBOrC,           //Filter by A | B | C condition.
};

typedef NS_ENUM(NSInteger, mk_cs_duplicateDataFilter) {
    mk_cs_duplicateDataFilter_none,
    mk_cs_duplicateDataFilter_mac,
    mk_cs_duplicateDataFilter_macAndDataType,
    mk_cs_duplicateDataFilter_macAndRawData
};

typedef NS_ENUM(NSInteger, mk_cs_duplicateDataFilterStrategy) {
    mk_cs_duplicateDataFilterStrategy1,
    mk_cs_duplicateDataFilterStrategy2,
};

typedef NS_ENUM(NSInteger, mk_cs_PHYMode) {
...
@protocol mk_cs_BLEFilterRawDataProtocol <NSObject>
...
@protocol mk_cs_BLEFilterBXPButtonProtocol <NSObject>
...
@protocol mk_cs_BLEFilterPirProtocol <NSObject>
...
@protocol mk_cs_mqttModifyWifiProtocol <NSObject>
...
@protocol mk_cs_mqttModifyWifiEapCertProtocol <NSObject>
...
@protocol mk_cs_mqttModifyNetworkProtocol <NSObject>
...
@protocol mk_cs_modifyMqttServerProtocol <NSObject>
...
@protocol mk_cs_modifyMqttServerCertsProtocol <NSObject>
...
@protocol cs_advertiseBeaconProtocol <NSObject>
