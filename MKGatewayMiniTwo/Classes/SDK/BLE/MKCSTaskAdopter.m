...
NSString *const mk_cs_totalNumKey = @"mk_cs_totalNumKey";
NSString *const mk_cs_totalIndexKey = @"mk_cs_totalIndexKey";
NSString *const mk_cs_contentKey = @"mk_cs_contentKey";

@implementation MKCSTaskAdopter
...
+ (NSDictionary *)parseReadDataWithCharacteristic:(CBCharacteristic *)characteristic {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A24"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"modeID":tempString} operationID:mk_cs_taskReadDeviceModelOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A26"]]) {
        //firmware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"firmware":tempString} operationID:mk_cs_taskReadFirmwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A27"]]) {
        //hardware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"hardware":tempString} operationID:mk_cs_taskReadHardwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A28"]]) {
        //soft ware
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"software":tempString} operationID:mk_cs_taskReadSoftwareOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A29"]]) {
        //manufacturerKey
        NSString *tempString = [[NSString alloc] initWithData:readData encoding:NSUTF8StringEncoding];
        return [self dataParserGetDataSuccess:@{@"manufacturer":tempString} operationID:mk_cs_taskReadManufacturerOperation];
    }
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA00"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"state":state} operationID:mk_cs_connectPasswordOperation];
    }
...
+ (NSDictionary *)parseCustomReadData:(NSString *)content cmd:(NSString *)cmd data:(NSData *)data {
    mk_cs_taskOperationID operationID = mk_cs_defaultTaskOperationID;
    ...
    if ([cmd isEqualToString:@"02"]) {
        ...
        operationID = mk_cs_taskReadDeviceNameOperation;
        ...
        operationID = mk_cs_taskReadDeviceMacAddressOperation;
    }else if ([cmd isEqualToString:@"0a"]) {
        //读取WIFI STA MAC
        NSString *macAddress = [NSString stringWithFormat:@"%@:%@:%@:%@:%@:%@",[content substringWithRange:NSMakeRange(0, 2)],[content substringWithRange:NSMakeRange(2, 2)],[content substringWithRange:NSMakeRange(4, 2)],[content substringWithRange:NSMakeRange(6, 2)],[content substringWithRange:NSMakeRange(8, 2)],[content substringWithRange:NSMakeRange(10, 2)]];
        resultDic = @{@"macAddress":[macAddress uppercaseString]};
        operationID = mk_cs_taskReadDeviceWifiSTAMacAddressOperation;
        ...
        operationID = mk_cs_taskReadNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"12"]) {
        //读取时区
        resultDic = @{
            @"timeZone":[MKBLEBaseSDKAdopter signedHexTurnString:content],
        };
        operationID = mk_cs_taskReadTimeZoneOperation;
        ...
        operationID = mk_cs_taskReadWifiFirmwareOperation;
    }else if ([cmd isEqualToString:@"16"]) {
        //读取BLE固件版本
        NSString *firmware = [[NSString alloc] initWithData:[data subdataWithRange:NSMakeRange(4, data.length - 4)] encoding:NSUTF8StringEncoding];
        resultDic = @{@"firmware":firmware};
        operationID = mk_cs_taskReadBLEFirmwareOperation;
        ...
        operationID = mk_cs_taskReadServerHostOperation;
        ...
        resultDic = @{@"port":port};
        operationID = mk_cs_taskReadServerPortOperation;
        ...
        operationID = mk_cs_taskReadServerKeepAliveOperation;
        ...
        resultDic = @{@"qos":qos};
        operationID = mk_cs_taskReadServerQosOperation;
        ...
        operationID = mk_cs_taskReadSubscibeTopicOperation;
        ...
        operationID = mk_cs_taskReadPublishTopicOperation;
    }else if ([cmd isEqualToString:@"2a"]) {
        //读取LWT 开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cs_taskReadLWTStatusOperation;
    }else if ([cmd isEqualToString:@"2b"]) {
        //读取LWT Qos
        NSString *qos = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"qos":qos};
        operationID = mk_cs_taskReadLWTQosOperation;
    }else if ([cmd isEqualToString:@"2c"]) {
        //读取LWT Retain
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cs_taskReadLWTRetainOperation;
        ...
        operationID = mk_cs_taskReadLWTTopicOperation;
        ...
        operationID = mk_cs_taskReadLWTPayloadOperation;
        ...
        resultDic = @{@"mode":mode};
        operationID = mk_cs_taskReadConnectModeOperation;
        ...
        operationID = mk_cs_taskReadWIFISecurityOperation;
        ...
        operationID = mk_cs_taskReadWIFISSIDOperation;
        ...
        operationID = mk_cs_taskReadWIFIPasswordOperation;
        ...
        operationID = mk_cs_taskReadWIFIEAPTypeOperation;
        ...
        operationID = mk_cs_taskReadWIFIEAPUsernameOperation;
        ...
        operationID = mk_cs_taskReadWIFIEAPPasswordOperation;
        ...
        operationID = mk_cs_taskReadWIFIEAPDomainIDOperation;
    }else if ([cmd isEqualToString:@"47"]) {
        //读取是否校验服务器
        BOOL verify = ([content isEqualToString:@"01"]);
        resultDic = @{@"verify":@(verify)};
        operationID = mk_cs_taskReadWIFIVerifyServerStatusOperation;
    }else if ([cmd isEqualToString:@"4b"]) {
        //读取DHCP开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cs_taskReadWIFIDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"4c"]) {
        //读取IP信息
        NSString *ip = [MKCSSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(0, 8)]];
        NSString *mask = [MKCSSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(8, 8)]];
        NSString *gateway = [MKCSSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(16, 8)]];
        NSString *dns = [MKCSSDKDataAdopter parseIpAddress:[content substringWithRange:NSMakeRange(24, 8)]];
        resultDic = @{
            @"ip":ip,
            @"mask":mask,
            @"gateway":gateway,
        ...
        operationID = mk_cs_taskReadWIFINetworkIpInfosOperation;
        ...
        operationID = mk_cs_taskReadRssiFilterValueOperation;
        ...
        operationID = mk_cs_taskReadFilterRelationshipOperation;
    }else if ([cmd isEqualToString:@"64"]) {
        //读取MAC过滤列表
        NSArray *macList = [MKCSSDKDataAdopter parseFilterMacList:content];
        resultDic = @{
            @"macList":(MKValidArray(macList) ? macList : @[]),
        };
        operationID = mk_cs_taskReadFilterMACAddressListOperation;
        ...
        operationID = mk_cs_taskReadFilterReportIntervalOperation;
    }else if ([cmd isEqualToString:@"70"]) {
        //读取iBeacon开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cs_taskReadAdvertiseBeaconStatusOperation;
        ...
        operationID = mk_cs_taskReadBeaconMajorOperation;
        ...
        operationID = mk_cs_taskReadBeaconMinorOperation;
    }else if ([cmd isEqualToString:@"73"]) {
        //读取iBeacon UUID
        resultDic = @{@"uuid":content};
        operationID = mk_cs_taskReadBeaconUUIDOperation;
    }else if ([cmd isEqualToString:@"74"]) {
        //读取广播频率
        NSString *interval = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"interval":interval};
        operationID = mk_cs_taskReadBeaconAdvIntervalOperation;
        ...
        operationID = mk_cs_taskReadBeaconTxPowerOperation;
    }else if ([cmd isEqualToString:@"76"]) {
        //读取RSSI@1m
        resultDic = @{
            @"rssi":[NSString stringWithFormat:@"%@",[MKBLEBaseSDKAdopter signedHexTurnString:content]],
        };
        operationID = mk_cs_taskReadBeaconRssiOperation;
    }else if ([cmd isEqualToString:@"77"]) {
        //读取可连接状态
        BOOL connectable = ([content isEqualToString:@"01"]);
        resultDic = @{@"connectable":@(connectable)};
        operationID = mk_cs_taskReadConnectableOperation;
    }else if ([cmd isEqualToString:@"80"]) {
        //读取计量数据上报开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cs_taskReadMeteringSwitchOperation;
    }else if ([cmd isEqualToString:@"81"]) {
        //读取电量数据上报间隔
        NSString *interval = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"interval":interval};
        operationID = mk_cs_taskReadPowerReportIntervalOperation;
    }else if ([cmd isEqualToString:@"82"]) {
        //读取电能数据上报间隔
        NSString *interval = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"interval":interval};
        operationID = mk_cs_taskReadEnergyReportIntervalOperation;
    }else if ([cmd isEqualToString:@"83"]) {
        //读取负载检测通知开关
        BOOL isOn = ([content isEqualToString:@"01"]);
        resultDic = @{@"isOn":@(isOn)};
        operationID = mk_cs_taskReadLoadDetectionNotificationStatusOperation;
    }else if ([cmd isEqualToString:@"c0"]) {
        //读取设备模式
        NSString *mode = [MKBLEBaseSDKAdopter getDecimalStringWithHex:content range:NSMakeRange(0, content.length)];
        resultDic = @{@"mode":mode};
        operationID = mk_cs_taskReadDeviceModeOperation;
    }
...
+ (NSDictionary *)parseCustomConfigData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"02"]) {
        //重启进入STA模式
        operationID = mk_cs_taskEnterSTAModeOperation;
    }else if ([cmd isEqualToString:@"11"]) {
        //配置NTP服务器域名
        operationID = mk_cs_taskConfigNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"12"]) {
        //配置时区
        operationID = mk_cs_taskConfigTimeZoneOperation;
    }else if ([cmd isEqualToString:@"20"]) {
        //配置MQTT服务器域名
        operationID = mk_cs_taskConfigServerHostOperation;
    }else if ([cmd isEqualToString:@"21"]) {
        //配置MQTT服务器端口
        operationID = mk_cs_taskConfigServerPortOperation;
    }else if ([cmd isEqualToString:@"22"]) {
        //配置ClientID
        operationID = mk_cs_taskConfigClientIDOperation;
    }else if ([cmd isEqualToString:@"25"]) {
        //配置MQTT Clean Session
        operationID = mk_cs_taskConfigServerCleanSessionOperation;
    }else if ([cmd isEqualToString:@"26"]) {
        //配置MQTT KeepAlive
        operationID = mk_cs_taskConfigServerKeepAliveOperation;
    }else if ([cmd isEqualToString:@"27"]) {
        //配置MQTT Qos
        operationID = mk_cs_taskConfigServerQosOperation;
    }else if ([cmd isEqualToString:@"28"]) {
        //配置Subscribe topic
        operationID = mk_cs_taskConfigSubscibeTopicOperation;
    }else if ([cmd isEqualToString:@"29"]) {
        //配置Publish topic
        operationID = mk_cs_taskConfigPublishTopicOperation;
    }else if ([cmd isEqualToString:@"2a"]) {
        //配置LWT 开关
        operationID = mk_cs_taskConfigLWTStatusOperation;
    }else if ([cmd isEqualToString:@"2b"]) {
        //配置LWT Qos
        operationID = mk_cs_taskConfigLWTQosOperation;
    }else if ([cmd isEqualToString:@"2c"]) {
        //配置LWT Retain
        operationID = mk_cs_taskConfigLWTRetainOperation;
    }else if ([cmd isEqualToString:@"2d"]) {
        //配置LWT topic
        operationID = mk_cs_taskConfigLWTTopicOperation;
    }else if ([cmd isEqualToString:@"2e"]) {
        //配置LWT payload
        operationID = mk_cs_taskConfigLWTPayloadOperation;
    }else if ([cmd isEqualToString:@"2f"]) {
        //配置MTQQ服务器通信加密方式
        operationID = mk_cs_taskConfigConnectModeOperation;
    }else if ([cmd isEqualToString:@"40"]) {
        //配置WIFI 加密模式
        operationID = mk_cs_taskConfigWIFISecurityOperation;
    }else if ([cmd isEqualToString:@"41"]) {
        //配置WIFI SSID
        operationID = mk_cs_taskConfigWIFISSIDOperation;
    }else if ([cmd isEqualToString:@"42"]) {
        //配置WIFI password
        operationID = mk_cs_taskConfigWIFIPasswordOperation;
    }else if ([cmd isEqualToString:@"43"]) {
        //配置WIFI EAP 类型
        operationID = mk_cs_taskConfigWIFIEAPTypeOperation;
    }else if ([cmd isEqualToString:@"44"]) {
        //配置WIFI EAP 用户名
        operationID = mk_cs_taskConfigWIFIEAPUsernameOperation;
    }else if ([cmd isEqualToString:@"45"]) {
        //配置WIFI EAP 密码
        operationID = mk_cs_taskConfigWIFIEAPPasswordOperation;
    }else if ([cmd isEqualToString:@"46"]) {
        //配置wifi的EAP域名ID
        operationID = mk_cs_taskConfigWIFIEAPDomainIDOperation;
    }else if ([cmd isEqualToString:@"47"]) {
        //配置wifi是否校验服务器
        operationID = mk_cs_taskConfigWIFIVerifyServerStatusOperation;
    }else if ([cmd isEqualToString:@"4b"]) {
        //配置Wifi DHCP状态
        operationID = mk_cs_taskConfigWIFIDHCPStatusOperation;
    }else if ([cmd isEqualToString:@"4c"]) {
        //配置Wifi IP地址相关信息
        operationID = mk_cs_taskConfigWIFIIpInfoOperation;
    }else if ([cmd isEqualToString:@"50"]) {
        //进行一次wifi扫描
        operationID = mk_cs_taskStartWifiScanOperation;
    }else if ([cmd isEqualToString:@"60"]) {
        //配置扫描RSSI过滤
        operationID = mk_cs_taskConfigRssiFilterValueOperation;
    }else if ([cmd isEqualToString:@"61"]) {
        //配置扫描过滤逻辑
        operationID = mk_cs_taskConfigFilterRelationshipOperation;
    }else if ([cmd isEqualToString:@"64"]) {
        //配置MAC过滤规则
        operationID = mk_cs_taskConfigFilterMACAddressListOperation;
    }else if ([cmd isEqualToString:@"69"]) {
        //配置数据上报间隔
        operationID = mk_cs_taskConfigFilterReportIntervalOperation;
    }else if ([cmd isEqualToString:@"70"]) {
        //配置iBeacon 开关
        operationID = mk_cs_taskConfigAdvertiseBeaconStatusOperation;
    }else if ([cmd isEqualToString:@"71"]) {
        //配置iBeacon major
        operationID = mk_cs_taskConfigBeaconMajorOperation;
    }else if ([cmd isEqualToString:@"72"]) {
        //配置iBeacon minor
        operationID = mk_cs_taskConfigBeaconMinorOperation;
    }else if ([cmd isEqualToString:@"73"]) {
        //配置iBeacon uuid
        operationID = mk_cs_taskConfigBeaconUUIDOperation;
    }else if ([cmd isEqualToString:@"74"]) {
        //配置广播频率
        operationID = mk_cs_taskConfigAdvIntervalOperation;
    }else if ([cmd isEqualToString:@"75"]) {
        //配置发射功率
        operationID = mk_cs_taskConfigTxPowerOperation;
    }else if ([cmd isEqualToString:@"76"]) {
        //配置Beacon Rssi@1m
        operationID = mk_cs_taskConfigBeaconRssiOperation;
    }else if ([cmd isEqualToString:@"77"]) {
        //配置可连接状态
        operationID = mk_cs_taskConfigConnectableOperation;
    }else if ([cmd isEqualToString:@"80"]) {
        //配置计量数据上报开关
        operationID = mk_cs_taskConfigMeteringSwitchOperation;
    }else if ([cmd isEqualToString:@"81"]) {
        //配置电量数据上报间隔
        operationID = mk_cs_taskConfigPowerReportIntervalOperation;
    }else if ([cmd isEqualToString:@"82"]) {
        //配置电能数据上报间隔
        operationID = mk_cs_taskConfigEnergyReportIntervalOperation;
    }else if ([cmd isEqualToString:@"83"]) {
        //配置负载检测通知开关
        operationID = mk_cs_taskConfigLoadDetectionNotificationStatusOperation;
    }
...
+ (NSDictionary *)parseMultiPackageReadData:(NSData *)readData cmd:(NSString *)cmd {
    ...
    NSDictionary *resultDic= @{
        mk_cs_totalNumKey:totalNum,
        mk_cs_totalIndexKey:index,
        mk_cs_contentKey:(subData ? subData : [NSData data]),
    };
    if ([cmd isEqualToString:@"23"]) {
        //读取服务器登录用户名
        operationID = mk_cs_taskReadServerUserNameOperation;
    }else if ([cmd isEqualToString:@"24"]) {
        //读取服务器登录密码
        operationID = mk_cs_taskReadServerPasswordOperation;
    }else if ([cmd isEqualToString:@"67"]) {
        //读取BLE Name过滤规则
        operationID = mk_cs_taskReadFilterAdvNameListOperation;
    }
    return [self dataParserGetDataSuccess:resultDic operationID:operationID];
}
...
+ (NSDictionary *)parseMultiPackageData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"23"]) {
        //配置服务器登录用户名
        operationID = mk_cs_taskConfigServerUserNameOperation;
    }else if ([cmd isEqualToString:@"24"]) {
        //配置服务器登录密码
        operationID = mk_cs_taskConfigServerPasswordOperation;
    }else if ([cmd isEqualToString:@"30"]) {
        //配置CA file
        operationID = mk_cs_taskConfigCAFileOperation;
    }else if ([cmd isEqualToString:@"31"]) {
        //配置Client certificate file
        operationID = mk_cs_taskConfigClientCertOperation;
    }else if ([cmd isEqualToString:@"32"]) {
        //配置Client key file
        operationID = mk_cs_taskConfigClientPrivateKeyOperation;
    }else if ([cmd isEqualToString:@"48"]) {
        //配置WIFI CA file
        operationID = mk_cs_taskConfigWIFICAFileOperation;
    }else if ([cmd isEqualToString:@"49"]) {
        //配置WIFI Client certificate file
        operationID = mk_cs_taskConfigWIFIClientCertOperation;
    }else if ([cmd isEqualToString:@"4a"]) {
        //配置WIFI Client key file
        operationID = mk_cs_taskConfigWIFIClientPrivateKeyOperation;
    }else if ([cmd isEqualToString:@"67"]) {
        //配置Adv Name过滤规则
        operationID = mk_cs_taskConfigFilterAdvNameListOperation;
    }
    return [self dataParserGetDataSuccess:@{@"success":@(success)} operationID:operationID];
}

+ (NSDictionary *)dataParserGetDataSuccess:(NSDictionary *)returnData operationID:(mk_cs_taskOperationID)operationID{
    if (!returnData) {
        return @{};
    }
