...
@implementation MKCSSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_cs_wifiSecurity)security {
    switch (security) {
        case mk_cs_wifiSecurity_personal:
            return @"00";
        case mk_cs_wifiSecurity_enterprise:
            return @"01";
    }
}
...
+ (NSString *)fetchWifiEapType:(mk_cs_eapType)eapType {
    switch (eapType) {
        case mk_cs_eapType_peap_mschapv2:
            return @"00";
        case mk_cs_eapType_ttls_mschapv2:
            return @"01";
        case mk_cs_eapType_tls:
            return @"02";
    }
}
...
+ (NSString *)fetchConnectModeString:(mk_cs_connectMode)mode {
    switch (mode) {
        case mk_cs_connectMode_TCP:
            return @"00";
        case mk_cs_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_cs_connectMode_CACertificate:
            return @"02";
        case mk_cs_connectMode_SelfSignedCertificates:
            return @"03";
    }
}
...
+ (NSString *)fetchMqttServerQosMode:(mk_cs_mqttServerQosMode)mode {
    switch (mode) {
        case mk_cs_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_cs_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_cs_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
