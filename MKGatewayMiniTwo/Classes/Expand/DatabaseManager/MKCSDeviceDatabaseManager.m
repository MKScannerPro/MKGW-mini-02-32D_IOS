...
@implementation MKCSDeviceDatabaseManager
...
+ (void)insertDeviceList:(NSArray <MKCSDeviceModel *>*)deviceList
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CSDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        for (MKCSDeviceModel *device in deviceList) {
            BOOL exist = NO;
...
+ (void)readLocalDeviceWithSucBlock:(void (^)(NSArray <MKCSDeviceModel *> *deviceList))sucBlock
                        failedBlock:(void (^)(NSError *error))failedBlock {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"CSDeviceDB")];
    if (![db open]) {
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"CSDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        while ([result next]) {
            MKCSDeviceModel *dataModel = [[MKCSDeviceModel alloc] init];
