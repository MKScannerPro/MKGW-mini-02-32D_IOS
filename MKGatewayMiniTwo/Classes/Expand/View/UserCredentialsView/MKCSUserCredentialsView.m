...
@implementation MKCSUserCredentialsViewModel
@end

@interface MKCSUserCredentialsView ()
...
@implementation MKCSUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKCSUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cs_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate cs_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(cs_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate cs_mqtt_userCredentials_passwordChanged:text];
            }
        };
