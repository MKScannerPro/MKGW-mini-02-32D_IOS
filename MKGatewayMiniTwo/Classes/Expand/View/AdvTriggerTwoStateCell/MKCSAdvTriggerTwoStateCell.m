...
@implementation MKCSAdvTriggerTwoStateCellModel
@end

@interface MKCSAdvTriggerTwoStateCell ()
...
@implementation MKCSAdvTriggerTwoStateCell

+ (MKCSAdvTriggerTwoStateCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSAdvTriggerTwoStateCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSAdvTriggerTwoStateCellIdenty"];
    if (!cell) {
        cell = [[MKCSAdvTriggerTwoStateCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSAdvTriggerTwoStateCellIdenty"];
    }
    return cell;
}
...
- (void)setButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cs_advNormalCell_setPressed:beforeInterval:beforeTxPower:afterInterval:afterTxPower:)]) {
        [self.delegate cs_advNormalCell_setPressed:self.dataModel.index
...
- (void)txPowerButtonPressed:(UIButton *)btn {
    ...
    [pickView showPickViewWithDataList:self.txPowerList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(cs_advNormalCell_txPowerChanged:before:txPower:)]) {
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKCSAdvTriggerTwoStateCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSAdvTriggerTwoStateCellModel.class]) {
        return;
    }
