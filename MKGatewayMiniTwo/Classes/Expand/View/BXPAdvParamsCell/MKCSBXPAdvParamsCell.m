...
@implementation MKCSBXPAdvParamsCellModel

- (CGFloat)fetchCellHeight {
    if (self.slotType == MKCSBXPAdvParamsCellSlotTypeNoData) {
        return 44.f;
    }
    return 150.f;
}
...
@interface MKCSBXPAdvParamsCell ()
...
@implementation MKCSBXPAdvParamsCell

+ (MKCSBXPAdvParamsCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSBXPAdvParamsCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSBXPAdvParamsCellIdenty"];
    if (!cell) {
        cell = [[MKCSBXPAdvParamsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSBXPAdvParamsCellIdenty"];
    }
    return cell;
}
...
- (void)layoutSubviews {
    ...
    if (self.dataModel.slotType == MKCSBXPAdvParamsCellSlotTypeNoData) {
...
- (void)setButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(cs_BXPAdvParamsCell_setPressedWithSlotIndex:interval:txPower:)]) {
        [self.delegate cs_BXPAdvParamsCell_setPressedWithSlotIndex:self.dataModel.slotIndex
...
#pragma mark - setter
- (void)setDataModel:(MKCSBXPAdvParamsCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSBXPAdvParamsCellModel.class]) {
        return;
    }
    ...
    if (_dataModel.slotType == MKCSBXPAdvParamsCellSlotTypeNoData) {
        [self setNeedsLayout];
        return;
    }
...
- (NSString *)getSlotTypeString:(MKCSBXPAdvParamsCellSlotType)slotType {
    if (slotType == MKCSBXPAdvParamsCellSlotTypeUID) {
        return @"UID";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeURL) {
        return @"URL";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeTLM) {
        return @"TLM";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeEID) {
        return @"EID";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeDeviceInfo) {
        return @"Device info";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeiBeacon) {
        return @"iBeacon";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeAccelerometer) {
        return @"ACC";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeHT) {
        return @"TH";
    }
    if (slotType == MKCSBXPAdvParamsCellSlotTypeTag) {
        return @"Tag";
    }
    return @"No data";
}

- (NSString *)getTriggerTypeString:(MKCSBXPAdvParamsCellTriggerType)triggerType {
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeNull) {
        return @"Off";
    }
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeTemperature) {
        return @"Temperature";
    }
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeHumidity) {
        return @"Humidity";
    }
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeDoubleClick) {
        return @"Double click button";
    }
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeTripleClick) {
        return @"Triple click button";
    }
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeMove) {
        return @"Devices moves";
    }
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeLight) {
        return @"Light";
    }
    if (triggerType == MKCSBXPAdvParamsCellTriggerTypeSingleClick) {
