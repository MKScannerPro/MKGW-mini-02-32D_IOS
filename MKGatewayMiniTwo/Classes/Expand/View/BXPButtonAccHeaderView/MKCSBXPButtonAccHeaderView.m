...
@interface MKCSBXPButtonAccHeaderView ()
...
@implementation MKCSBXPButtonAccHeaderView
...
- (void)syncButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_bxpButtonAccHeaderView_syncButtonPressed:)]) {
        [self.delegate cs_bxpButtonAccHeaderView_syncButtonPressed:!self.syncButton.selected];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_bxpButtonAccHeaderView_exportButtonPressed)]) {
        [self.delegate cs_bxpButtonAccHeaderView_exportButtonPressed];
    }
}
...
- (void)updateSyncStatus:(BOOL)isOn {
    [self.syncIcon.layer removeAnimationForKey:@"mk_cs_syncAnimation"];
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPButtonAccHeaderView", @"cs_threeAxisAcceLoadingIcon.png");
    }
    return _syncIcon;
}
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKGatewayMiniTwo", @"MKCSBXPButtonAccHeaderView", @"cs_slotExportEnableIcon.png") forState:UIControlStateNormal];
