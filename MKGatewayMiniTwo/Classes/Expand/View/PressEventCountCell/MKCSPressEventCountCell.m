...
@implementation MKCSPressEventCountCellModel
@end

@interface MKCSPressEventCountCell ()
...
@implementation MKCSPressEventCountCell

+ (MKCSPressEventCountCell *)initCellWithTableView:(UITableView *)tableView {
    MKCSPressEventCountCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKCSPressEventCountCellIdenty"];
    if (!cell) {
        cell = [[MKCSPressEventCountCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKCSPressEventCountCellIdenty"];
    }
    return cell;
}
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(cs_pressEventCountCell_clearButtonPressed:)]) {
        [self.delegate cs_pressEventCountCell_clearButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKCSPressEventCountCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCSPressEventCountCellModel.class]) {
        return;
    }
